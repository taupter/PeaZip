unit UnitGwrap; //PeaLauncher graphic wrapper form, displaying archiving/extraction/test tasks progress and status (formerly compiled as separate binary)
{
 DESCRIPTION     :  PeaLauncher graphic wrapper for CLI backend tasks, component
                    launching console tools and providing GUI for:
                    displaying output of console using pipes;
                    displaying and explaining job exitcode.
 ...
 1.15     20250820  G.Tani      Legacy code clanup
...
uses
  ...
  {$ENDIF}
  UnitDlg, Classes, SysUtils, LResources, Forms, Controls, Graphics, Dialogs, ExtCtrls, strutils,
  StdCtrls, Buttons, Process, UTF8Process, Menus,
  pea_utils, list_utils, img_utils, Grids, Math, externalprograms;

type

  { TFormGwrap }

  TFormGwrap = class(TForm)
    ...
    MenuItemSecuredelete: TMenuItem;
    pmmactm: TMenuItem;
    pmgnometm: TMenuItem;
    pmkdetm: TMenuItem;
    pmtop: TMenuItem;
    ...
    pmexplore: TMenuItem;
    pmsearch: TMenuItem;
    PanelAlignBottom: TPanel;
    PopupMenuTask: TPopupMenu;
    PopupMenuTray: TPopupMenu;
    SaveDialogTask: TSaveDialog;
    ShapeProgress1: TShape;
    ShapeProgress10: TShape;
    ShapeProgress11: TShape;
    ShapeProgress12: TShape;
    ShapeProgress2: TShape;
    ShapeProgress3: TShape;
    ShapeProgress4: TShape;
    ShapeProgress5: TShape;
    ShapeProgress6: TShape;
    ShapeProgress7: TShape;
    ShapeProgress8: TShape;
    ShapeProgress9: TShape;
    ...
    procedure LabelKeepClick(Sender: TObject);
    procedure LabelSavePJClick(Sender: TObject);
    procedure ButtonOKClick(Sender: TObject);
    procedure LabelSaveReportClick(Sender: TObject);
    ...
    procedure LabelWarningClick(Sender: TObject);
    procedure LabelDeleteInputClick(Sender: TObject);
    procedure MenuItemRecycleClick(Sender: TObject);
    procedure MenuItemQuickdeleteClick(Sender: TObject);
    procedure MenuItemZerodeleteClick(Sender: TObject);
    procedure MenuItemSecuredeleteClick(Sender: TObject);
    ...
    procedure TrayIconTaskClick(Sender: TObject);
    procedure TrayIconTaskDblClick(Sender: TObject);
  private
...
var
  ...
  desk_env,pcount,optype,filesizebase,pautoclose,perasepasses:byte;
  T,conf:text;
  f:file of byte;
  tsin:TTimestamp;
...
with FormGwrap do
begin
LabelSaveReport.visible:=false;
LabelSavePJ.visible:=false;
LabelSpacer.visible:=false;
LabelKeep.visible:=false;
case i of
   1: begin
      PageStatus1.Visible:=true;
      PageReport2.Visible:=false;
      PageConsole3.Visible:=false;
      PageOpt4.Visible:=false;
      if LabelKeep.Caption=txt_8_2_keep then
         begin
         if LabelWarning.visible=true then LabelSpacer.Visible:=true;
         LabelKeep.visible:=true;
         end;
      end;
   2: begin
      PageStatus1.Visible:=false;
      PageReport2.Visible:=true;
      PageConsole3.Visible:=false;
      PageOpt4.Visible:=false;
      if LabelWarning.Visible=true then LabelSpacer.Visible:=true;
      LabelSaveReport.visible:=true;
      end;
   3: begin
      PageStatus1.Visible:=false;
      PageReport2.Visible:=false;
      PageConsole3.Visible:=true;
      PageOpt4.Visible:=false;
      if LabelWarning.Visible=true then LabelSpacer.Visible:=true;
      LabelSavePJ.visible:=true;
      end;
   4: begin
      PageStatus1.Visible:=false;
      PageReport2.Visible:=false;
      PageConsole3.Visible:=false;
      PageOpt4.Visible:=true;
      end;
   end;
end;
end;
...
begin
UnitDlg.txt_info:=txt_5_3_info;
UnitDlg.txt_2_7_ok:=txt_ok;
UnitDlg.txt_2_7_cancel:=txt_2_3_cancel;
...
LabelKeep.Caption:='';
LabelKeep.Hint:='';
LabelWarning.Caption:=txt_3_0_hints;
...
LabelSavePJ.Caption:=txt_savejob;
LabelSaveReport.Caption:=txt_savelog;
LabelBench1.Hint:=txt_benchscale;
SaveDialogTask.Title:=txt_saveas;
...
if cp_open<33 then
   cp_open:=shellexecuteW(FormGwrap.handle,PWideChar('open'),PWideChar('RUNDLL32.EXE'),PWideChar('shell32.dll,OpenAs_RunDLL '+w),PWideChar (''), SW_SHOWNORMAL);
{$ENDIF}
...
FormGwrap.SaveDialogTask.FileName:=s;
if outpath<>'' then
   if checkdirexists(outpath) then FormGwrap.SaveDialogTask.InitialDir:=outpath
   else FormGwrap.SaveDialogTask.InitialDir:=extractfilepath(outpath);
if FormGwrap.SaveDialogTask.Execute then
   begin
   s:=FormGwrap.SaveDialogTask.FileName;
   assignfile(t,(s));
   rewrite(t);
   write_header(t);
   ...
   if modeofuse=2 then
      begin
      writeln(t,'');
      writeln(t,FormGwrap.LabelBench1.Caption);
      writeln(t,FormGwrap.LabelBench2.Caption);
      writeln(t,FormGwrap.LabelBench3.Caption);
      writeln(t,FormGwrap.LabelBench4.Caption);
      end;
   closefile(t);
   p:=(getcurrentdir);
   if p<>'' then
...
if umode=0 then //not 7z / p7zip
   ...
   iprog:=(FormGwrap.Width*i div 1000000);
   if iprog<3 then iprog:=3;
   ShapeProgress.Width:=iprog;
   if FormGwrap.Width<>0 then
      if ShapeProgress.Width<>0 then
         iperc:=(ShapeProgress.Width *100) div FormGwrap.Width;
   end
else //7z / p7zip
   begin
   if FormGwrap.Width<>0 then
      if (iperc>0) and (iperc<100) then
         ShapeProgress.Width:=(FormGwrap.Width*iperc) div 100;
   tpath:=outpath;
   if (optype=1) and (modeofuse=0) then
      if fileexists(tpath+'.tmp') then tpath:=outpath+'.tmp';
   try
...
{$ENDIF}
if FormGwrap.Visible=true then setwrap;
end;
end;

procedure setiomenu;
begin
if FormGwrap.l2.caption='' then FormGwrap.pmei.visible:=false else FormGwrap.pmei.visible:=true;
if poutnamet='' then FormGwrap.pmet.visible:=false else FormGwrap.pmet.visible:=true;
if FormGwrap.l4.caption='' then FormGwrap.pmeo.visible:=false else FormGwrap.pmeo.visible:=true;
FormGwrap.pm2ei.visible:=FormGwrap.pmei.visible;
FormGwrap.pm2et.visible:=FormGwrap.pmet.visible;
FormGwrap.pm2eo.visible:=FormGwrap.pmeo.visible;
FormGwrap.pmei.caption:=txt_explore+' '+FormGwrap.l2.caption;
FormGwrap.pmet.caption:=txt_explore+' TMP';
FormGwrap.pmeo.caption:=txt_explore+' '+FormGwrap.l4.caption;
FormGwrap.pm2ei.caption:=FormGwrap.pmei.caption;
FormGwrap.pm2et.caption:=FormGwrap.pmet.caption;
FormGwrap.pm2eo.caption:=FormGwrap.pmeo.caption;
end;

procedure set_form_title;
var s,s1,s2:ansistring;
begin
FormGwrap.l1.Visible:=false;
FormGwrap.l2.Visible:=false;
FormGwrap.l3.Visible:=false;
FormGwrap.l4.Visible:=false;
FormGwrap.l1.Caption:='';
FormGwrap.l2.Caption:='';
...
if pfromnativedrag=true then
   ...
   FormGwrap.l3.Visible:=false;
   FormGwrap.l4.Visible:=false;
   FormGwrap.LabelAction.Caption:=txt_7_8_dd;
   FormGwrap.pmeo.Visible:=false;
   FormGwrap.pm2eo.Visible:=false;
   end;
setiomenu;
end;
...
with FormGwrap do
   begin
   if ppause=true then l6.Caption:=txt_paused+' '
   else l6.Caption:=txt_running+' ';
   case ppriority of
      0: l6.Caption:=l6.Caption+txt_rt;
      1: l6.Caption:=l6.Caption+txt_high;
      2: l6.Caption:=l6.Caption+txt_normal;
      3: l6.Caption:=l6.Caption+txt_idle;
      end;
   end;
FormGwrap.l7.Caption:=iact+txt_input+' '+FormGwrap.LabelInfo1.Caption+' '+txt_output+' '+FormGwrap.LabelInfo2.Caption;
FormGwrap.ImageTask.Hint:=FormGwrap.l6.Caption;
end;
...
FormGwrap.MemoTmp.Clear;
FormGwrap.MemoTmp.Append(stri2);
FormGwrap.StringGridReport.Rowcount:=FormGwrap.MemoTmp.Lines.Count;
for i:=0 to FormGwrap.MemoTmp.Lines.Count-1 do FormGwrap.StringGridReport.Cells[0,i]:=FormGwrap.MemoTmp.Lines[i];
FormGwrap.MemoTmp.Clear;
FormGwrap.StringGridReport.RowCount:=FormGwrap.StringGridReport.Rowcount-1; //last row may be incomplete
FormGwrap.StringGridReport.Row:=FormGwrap.StringGridReport.Rowcount-1;
...
rc:=FormGwrap.StringGridReport.Rowcount;
while stopinfo=false do
begin
i:=i+1;
if i>=rc-1 then stopinfo:=true;
dummystr:=FormGwrap.StringGridReport.Cells[0,i];
...
//original size
stopinfo:=false;
dummystr:=FormGwrap.StringGridReport.Cells[0,rc-2];
...
if i>1 then
   ...
   if rc>4 then
   s:=FormGwrap.StringGridReport.Cells[0,rc-4]+char($0A)+char($0D)+
   FormGwrap.StringGridReport.Cells[0,rc-3]+char($0A)+char($0D)+
   FormGwrap.StringGridReport.Cells[0,rc-2]+char($0A)+char($0D)+
   FormGwrap.StringGridReport.Cells[0,rc-1]
   else s:='';
   s1:='';
end;
//presentation
...
begin
FormGwrap.ButtonOK.Visible:=false;
FormGwrap.ButtonPause.Visible:=true;
FormGwrap.ButtonStop.Visible:=true;
if pprogn<>'' then
   FormGwrap.ButtonStopAll.Visible:=true
else
   FormGwrap.ButtonStopAll.Visible:=false;
if FormGwrap.ButtonStopAll.visible=true then
   begin
   FormGwrap.ButtonStopAll.AnchorSideRight.Control:=FormGwrap.ButtonStop;
   FormGwrap.ButtonOptions.AnchorSideRight.Control:=FormGwrap.ButtonStopAll;
   end
else
   FormGwrap.ButtonOptions.AnchorSideRight.Control:=FormGwrap.ButtonStop;
FormGwrap.pm2cancelall.Visible:=FormGwrap.ButtonStopAll.Visible;
end;

procedure setbuttonsclose;
begin
FormGwrap.ButtonOK.Visible:=true;
FormGwrap.ButtonPause.Visible:=false;
FormGwrap.ButtonStop.Visible:=false;
if FormGwrap.ButtonStopAll.visible=true then
   begin
   FormGwrap.ButtonStopAll.AnchorSideRight.Control:=FormGwrap.ButtonOK;
   FormGwrap.ButtonOptions.AnchorSideRight.Control:=FormGwrap.ButtonStopAll;
   end
else
   FormGwrap.ButtonOptions.AnchorSideRight.Control:=FormGwrap.ButtonOK;
end;

procedure savegwpos;
begin
if FormGwrap.Visible=true then
   if FormGwrap.WindowState=wsNormal then
      begin
      ws_gw_left:=FormGwrap.left;
      ws_gw_top:=FormGwrap.top;
      ws_gw_width:=FormGwrap.width;
      ws_gw_height:=FormGwrap.height;
      end;
end;
...
if (pproglast=true) or (FormGwrap.ShapeGlobalProgress.Visible=false) then
   ...
   FormGwrap.Visible:=false;
   exbackground:=false;
   FormGwrap.TrayIconTask.visible:=false;
   end;
end;
...
gocancelall:=false;
setbuttonsclose;
if FormGwrap.visible=false then FormGwrap.visible:=true;
if FormGwrap.WindowState=wsMinimized then FormGwrap.WindowState:=wsNormal;
if FormGwrap.Visible=true then FormGwrap.SetFocus;
application.ProcessMessages;
exbackground:=false;
FormGwrap.TrayIconTask.visible:=false;
...
begin
result:=FormGwrap.LabelTitle1.Width+
FormGwrap.LabelTitle2.Width+
FormGwrap.LabelTitle3.Width+
FormGwrap.LabelTitle1.BorderSpacing.Left*4;
if ntabs=4 then result:=result+FormGwrap.LabelTitle4.Width+FormGwrap.LabelTitle4.BorderSpacing.Left;
end;
...
begin
pstarted:=true;
FormGwrap.PanelTask.Visible:=true;
FormGwrap.l6.visible:=false;
FormGwrap.ImageTask.hint:='';
FormGwrap.PanelBench.Visible:=false;
FormGwrap.Shapeprogress.visible:=true;
FormGwrap.ShapeProgress.Width:=3;
progress:=0;
iact:='';
FormGwrap.MemoConsole.Clear;
FormGwrap.MemoConsole.Lines.Append(cl);
//FormGwrap.LabelTitle4.Visible:=false;
FormGwrap.LabelDeleteInput.Visible:=false;
ipercp:=0;
iperc:=0;
remtime:=0;
FormGwrap.shapeProgress12.Height:=2;
FormGwrap.shapeProgress11.Height:=2;
FormGwrap.shapeProgress10.Height:=2;
FormGwrap.shapeProgress9.Height:=2;
FormGwrap.shapeProgress8.Height:=2;
FormGwrap.shapeProgress7.Height:=2;
FormGwrap.shapeProgress6.Height:=2;
FormGwrap.shapeProgress5.Height:=2;
FormGwrap.shapeProgress4.Height:=2;
FormGwrap.shapeProgress3.Height:=2;
FormGwrap.shapeProgress2.Height:=2;
FormGwrap.shapeProgress1.Height:=random(pbarhsmall*3)+2;
FormGwrap.shapeProgress12.Visible:=true;
FormGwrap.shapeProgress11.Visible:=true;
FormGwrap.shapeProgress10.Visible:=true;
FormGwrap.shapeProgress9.Visible:=true;
FormGwrap.shapeProgress8.Visible:=true;
FormGwrap.shapeProgress7.Visible:=true;
FormGwrap.shapeProgress6.Visible:=true;
FormGwrap.shapeProgress5.Visible:=true;
FormGwrap.shapeProgress4.Visible:=true;
FormGwrap.shapeProgress3.Visible:=true;
FormGwrap.shapeProgress2.Visible:=true;
FormGwrap.shapeProgress1.Visible:=true;
if cl='' then
   begin
...
if ppause=false then
   begin
   FormGwrap.ShapeProgress.Color:=PGREEN;
   FormGwrap.ShapeGlobalProgress.Color:=PGREEN;
   end;
FormGwrap.LabelTitle1.caption:='      '+txt_isrunning+'      ';
FormGwrap.PanelTitlePLTabAlign.Width:=getalignw(4);
if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_launcher(FormGwrap.LabelTitle1,FormGwrap.ShapeTitleb1) else clicklabel_launcher(FormGwrap.LabelTitle1,FormGwrap.Shapelink1);
if insize>0 then
   if (pinsize>0) and (pinsize<>insize) then FormGwrap.LabelInfo1.Caption:=nicenumber(inttostr(pinsize),filesizebase)+' / '+nicenumber(inttostr(insize),filesizebase)
   else FormGwrap.LabelInfo1.Caption:=nicenumber(inttostr(insize),filesizebase)
else FormGwrap.LabelInfo1.Caption:='--';//nicenumber(inttostr(pinsize),filesizebase);
set_form_title;
...
{if runelevated=true then //9.9 requests elevation of peazip and restart so it applies to any function
   begin
   {$IFDEF MSWINDOWS}
   fe:='0';
   seiexit:=127;
   ...
   exit_code:=seiexit;
   FormGwrap.StringGrid1.Rowcount:=1;
   {$ENDIF}
   ...
   FormGwrap.TimerProgress.enabled:=true;
   case ppriority of
      0: begin
   ...
   if stopped=true then
      ...
      if okseven=true then
         try
         FormGwrap.FTaskBarList.SetProgressState(FormGwrap.AppHandle, TBPF_ERROR);
         except
         end;
      {$ENDIF}
      FormGwrap.ShapeProgress.Color:=PRED;
      FormGwrap.ShapeGlobalProgress.Color:=PRED;
      ppause:=false;
      break;
      end;
   if ppause=true then
...
pstarted:=false;
tsout:=datetimetotimestamp(now);
FormGwrap.ShapeProgress.Width:=FormGwrap.Width-6;//process terminated
if pproglast=true then FormGwrap.ShapeGlobalProgress.Width:=FormGwrap.Width-6;
Application.ProcessMessages;
      ...
      if modeofuse>=20 then
         ...
         begin
            ...
            P.Free;
            FormGwrap.MemoTmp.Clear;
            FormGwrap.MemoTmp.Lines.LoadFromStream(M);
            {SetString(stri, M.Memory, M.Size);
            ...
            M.Free;
            FormGwrap.StringGridReport.BeginUpdate;
            if FormGwrap.MemoTmp.Lines.Count>0 then
               begin
               FormGwrap.StringGridReport.Rowcount:=FormGwrap.MemoTmp.Lines.Count+1;
               for i:=0 to FormGwrap.MemoTmp.Lines.Count-1 do FormGwrap.StringGridReport.Cells[0,i]:=FormGwrap.MemoTmp.Lines[i];
               FormGwrap.MemoTmp.Clear;
               i:=i+1;
               FormGwrap.StringGridReport.Cells[0,i]:='';
               if FormGwrap.StringGridReport.RowCount<128*1024 then FormGwrap.StringGridReport.AutosizeColumns;
               FormGwrap.StringGridReport.Row:=FormGwrap.StringGridReport.Rowcount-1;
               end
            else
               begin
               FormGwrap.StringGridReport.Rowcount:=1;
               FormGwrap.StringGridReport.Cells[0,0]:='';
               end;
            FormGwrap.StringGridReport.EndUpdate;
         end;

FormGwrap.StringGridReport.Cursor:=crDefault;
FormGwrap.ShapeProgress.Width:=FormGwrap.Width-3; //read from memory stream, if used
if pproglast=true then FormGwrap.ShapeGlobalProgress.Width:=FormGwrap.Width-3;
Application.ProcessMessages;
except
...
tdiff:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
if tdiff<=0 then tdiff:=100000;
if pproglast=true then if exit_code<>0 then FormGwrap.LabelTitle1.caption:='      '+txt_status+'      ';
FormGwrap.PanelTitlePLTabAlign.Width:=getalignw(4);
if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_launcher(FormGwrap.LabelTitle1,FormGwrap.ShapeTitleb1) else clicklabel_launcher(FormGwrap.LabelTitle1,FormGwrap.Shapelink1);
{$IFNDEF MSWINDOWS}
...
if exit_code<>0 then
   begin
   perrors:=perrors+1;
   FormGwrap.l6.Caption:=s;
   FormGwrap.l6.visible:=true;
   end;
...
if modeofuse>=20 then
   begin
   FormGwrap.StringGridReport.Rowcount:=5;
   FormGwrap.StringGridReport.Cells[0,0]:=datetimetostr(timestamptodatetime(tsout))+' - '+s;
...
FormGwrap.shapeProgress12.Height:=0;
FormGwrap.shapeProgress11.Height:=0;
FormGwrap.shapeProgress10.Height:=0;
FormGwrap.shapeProgress9.Height:=0;
FormGwrap.shapeProgress8.Height:=0;
FormGwrap.shapeProgress7.Height:=0;
FormGwrap.shapeProgress6.Height:=0;
FormGwrap.shapeProgress5.Height:=0;
FormGwrap.shapeProgress4.Height:=0;
FormGwrap.shapeProgress3.Height:=0;
FormGwrap.shapeProgress2.Height:=0;
FormGwrap.shapeProgress1.Height:=0;
FormGwrap.shapeProgress12.Visible:=false;
FormGwrap.shapeProgress11.Visible:=false;
FormGwrap.shapeProgress10.Visible:=false;
FormGwrap.shapeProgress9.Visible:=false;
FormGwrap.shapeProgress8.Visible:=false;
FormGwrap.shapeProgress7.Visible:=false;
FormGwrap.shapeProgress6.Visible:=false;
FormGwrap.shapeProgress5.Visible:=false;
FormGwrap.shapeProgress4.Visible:=false;
FormGwrap.shapeProgress3.Visible:=false;
...
case exit_code of
   ...
   255: FormGwrap.Caption:=pprogn+' '+txt_halted+' '+pcapt;
      ...
      if (modeofuse<>2) and (modeofuse<>3) then
         ...
         FormGwrap.LabelSpacer.visible:=true;
         end;
      end;
   end;
FormGwrap.TrayIconTask.Hint:=FormGwrap.Caption;
...
case modeofuse of
   ...
   1,4,5: begin
      if (pfun<>'UNBROTLI') and (pfun<>'UNZSTD') then
            if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_launcher(FormGwrap.LabelTitle2,FormGwrap.ShapeTitleb2) else clicklabel_launcher(FormGwrap.LabelTitle2,FormGwrap.Shapelink2);
      end;
   2: begin
      FormGwrap.PanelTask.Visible:=false;
      FormGwrap.PanelBench.Visible:=true;
      try
      FormGwrap.LabelBench1.Caption:=FormGwrap.StringGridReport.Cells[0,6];
      FormGwrap.LabelBench2.Caption:=FormGwrap.StringGridReport.Cells[0,7];
      FormGwrap.LabelBench3.Caption:=FormGwrap.StringGridReport.Cells[0,5];
      FormGwrap.LabelBench4.Caption:='Rating: '+copy(FormGwrap.StringGridReport.Cells[0,FormGwrap.StringGridReport.RowCount-2],28,7);
      except
      end;
      end;
   3: begin
...
FormGwrap.ShapeProgress.Width:=FormGwrap.Width;
if FormGwrap.ShapeGlobalProgress.visible=true then FormGwrap.ShapeGlobalProgress.Width:=pprogbar;
if pproglast=true then FormGwrap.ShapeGlobalProgress.Width:=FormGwrap.Width;
ended:=true;
Application.ProcessMessages;
//FormGwrap.StringGrid1.AutosizeColumns;
//set label to optionally keep extracted content if errors are detected, if applicable
if FormGwrap.PageStatus1.Visible=true then FormGwrap.LabelSpacer.Visible:=false;
if exit_code<>0 then
   if (pcanignore=true) and (pfun='UN7Z') then
      begin
      FormGwrap.LabelKeep.Visible:=true;
      FormGwrap.LabelKeep.Caption:=txt_8_2_keep;
      FormGwrap.LabelKeep.Hint:=txt_10_7_keep;
      if FormGwrap.LabelWarning.visible=true then FormGwrap.LabelSpacer.Visible:=true;
      end;
//final actions, options
if (pproglast=true) or (pprogn='') then
begin
{if FormGwrap.CheckBoxHalt.State=cbChecked then //setting shutdown from progress window was disabled
   begin
   P:=TProcessUTF8.Create(nil);
   {$IFDEF MSWINDOWS}
   cl:='shutdown /s /t 10';
...
//FormGwrap.LabelTitle4.Visible:=true;
FormGwrap.LabelDeleteInput.caption:=txt_2_5_delete+' '+ExtractFileName(in_name);
if in_name='' then FormGwrap.LabelDeleteInput.visible:=false else FormGwrap.LabelDeleteInput.visible:=true;
FormGwrap.PanelTitlePLTabAlign.Width:=getalignw(4);
end;
...
procedure dograph;
begin
FormGwrap.shapeProgress12.Height:=FormGwrap.shapeProgress11.Height;
FormGwrap.shapeProgress11.Height:=FormGwrap.shapeProgress10.Height;
FormGwrap.shapeProgress10.Height:=FormGwrap.shapeProgress9.Height;
FormGwrap.shapeProgress9.Height:=FormGwrap.shapeProgress8.Height;
FormGwrap.shapeProgress8.Height:=FormGwrap.shapeProgress7.Height;
FormGwrap.shapeProgress7.Height:=FormGwrap.shapeProgress6.Height;
FormGwrap.shapeProgress6.Height:=FormGwrap.shapeProgress5.Height;
FormGwrap.shapeProgress5.Height:=FormGwrap.shapeProgress4.Height;
FormGwrap.shapeProgress4.Height:=FormGwrap.shapeProgress3.Height;
FormGwrap.shapeProgress3.Height:=FormGwrap.shapeProgress2.Height;
FormGwrap.shapeProgress2.Height:=FormGwrap.shapeProgress1.Height;
instdelta:=iperc-instperc;
instperc:=iperc;
if instdelta=0 then
   begin
   if FormGwrap.shapeProgress2.Height<3 then instdelta:=4
   else instdelta:=0;
   end
else
   if instdelta=1 then instdelta:=pbarhsmall*2
   else
      if FormGwrap.shapeProgress2.Height=pbarhsmall*4 then instdelta:=pbarhsmall*3
      else instdelta:=pbarhsmall*4;
instdelta:=(instdelta+FormGwrap.shapeProgress2.Height) div 2;
if instdelta mod 2 <> 0 then instdelta:=instdelta+1;
FormGwrap.shapeProgress1.Height:=instdelta;
Application.ProcessMessages;
end;

procedure TFormGwrap.TimerProgressTimer(Sender: TObject);
begin
...
if pstarted=true then
   begin
      ...
      {case pcount of
         1: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp2; pcount:=2; end;
         2: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp3; pcount:=3; end;
         3: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp4; pcount:=4; end;
         4: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp5; pcount:=5; end;
         5: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp6; pcount:=6; end;
         6: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp7; pcount:=7; end;
         7: begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp8; pcount:=8; end;
         else begin FormGwrap.Imagestatus.Picture.Bitmap:=Bp1; pcount:=1; end;
         end;}
      if pcount mod 4 = 0 then dograph;
   end;
end;

procedure TFormGwrap.TrayIconTaskClick(Sender: TObject);
begin
exbackground:=false;
TrayIconTask.visible:=false;
FormGwrap.visible:=true;
end;

procedure TFormGwrap.TrayIconTaskDblClick(Sender: TObject);
begin
exbackground:=false;
TrayIconTask.visible:=false;
FormGwrap.visible:=true;
...
procedure TFormGwrap.ButtonOKClick(Sender: TObject);
begin
go_ok;
end;

procedure TFormGwrap.LabelSavePJClick(Sender: TObject);
begin
save_cl;
end;

procedure TFormGwrap.ButtonOptionsClick(Sender: TObject);
var
   p:TPoint;
begin
p.x:=ButtonOptions.Left;
p.y:=ButtonOptions.top+ButtonOptions.Height;
p:=clienttoscreen(p);
PopupMenuTask.popup(p.x,p.y);
end;
...
procedure TFormGwrap.ButtonStopAllClick(Sender: TObject);
begin
gostopall;
end;

procedure TFormGwrap.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
needinteraction:=false;
...
procedure TFormGwrap.FormDestroy(Sender: TObject);
begin
{$IFDEF MSWINDOWS}CloseHandle(psem);{$ENDIF}
ExitCode:=exit_code;
end;

procedure TFormGwrap.FormResize(Sender: TObject);
...
FormGwrap.ShapeTitleb1.Brush.Color:=StringToColor(COLLOW);
FormGwrap.ShapeTitleb2.Brush.Color:=StringToColor(COLLOW);
FormGwrap.ShapeTitleb3.Brush.Color:=StringToColor(COLLOW);
FormGwrap.ShapeTitleb4.Brush.Color:=StringToColor(COLLOW);
FormGwrap.ShapeProgress1.Brush.Color:=(pgray);
FormGwrap.ShapeProgress2.Brush.Color:=(pgray);
FormGwrap.ShapeProgress3.Brush.Color:=(pgray);
FormGwrap.ShapeProgress4.Brush.Color:=(pgray);
FormGwrap.ShapeProgress5.Brush.Color:=(pgray);
FormGwrap.ShapeProgress6.Brush.Color:=(pgray);
FormGwrap.ShapeProgress7.Brush.Color:=(pgray);
FormGwrap.ShapeProgress8.Brush.Color:=(pgray);
FormGwrap.shapeProgress9.Brush.Color:=(pgray);
FormGwrap.shapeProgress10.Brush.Color:=(pgray);
FormGwrap.shapeProgress11.Brush.Color:=(pgray);
FormGwrap.shapeProgress12.Brush.Color:=(pgray);
FormGwrap.ShapeProgress1.Pen.Color:=(pgray);
FormGwrap.ShapeProgress2.Pen.Color:=(pgray);
FormGwrap.ShapeProgress3.Pen.Color:=(pgray);
FormGwrap.ShapeProgress4.Pen.Color:=(pgray);
FormGwrap.ShapeProgress5.Pen.Color:=(pgray);
FormGwrap.ShapeProgress6.Pen.Color:=(pgray);
FormGwrap.ShapeProgress7.Pen.Color:=(pgray);
FormGwrap.ShapeProgress8.Pen.Color:=(pgray);
FormGwrap.shapeProgress9.Pen.Color:=(pgray);
FormGwrap.shapeProgress10.Pen.Color:=(pgray);
FormGwrap.shapeProgress11.Pen.Color:=(pgray);
FormGwrap.shapeProgress12.Pen.Color:=(pgray);
end;

procedure TFormGwrap.FormShow(Sender: TObject);
begin
...
procedure TFormGwrap.LabelKeepClick(Sender: TObject);
begin
if pcanignore=true then perrignore:=true;
go_ok;
end;

procedure TFormGwrap.LabelSaveReportClick(Sender: TObject);
begin
save_report;
end;

procedure TFormGwrap.ButtonStopClick(Sender: TObject);
begin
stopped:=true;
end;
...
if ppause=false then
   ...
   if okseven=true then
      try
      FormGwrap.FTaskBarList.SetProgressState(FormGwrap.AppHandle, TBPF_Normal);
      except
      end;
   {$ENDIF}
   FormGwrap.ButtonPause.Caption:='   '+txt_pause+'   ';
   FormGwrap.pm2pause.Caption:=txt_pause;
   FormGwrap.LabelTitle1.caption:='      '+txt_isrunning+'      ';
   FormGwrap.PanelTitlePLTabAlign.Width:=getalignw(4);
   FormGwrap.ShapeProgress.Color:=PGREEN;
   FormGwrap.ShapeGlobalProgress.Color:=PGREEN;
   ...
   if okseven=true then
      try
      FormGwrap.FTaskBarList.SetProgressState(FormGwrap.AppHandle, TBPF_PAUSED);
      except
      end;
   {$ENDIF}
   FormGwrap.ButtonPause.Caption:='   '+txt_resume+'   ';
   FormGwrap.pm2pause.Caption:=txt_resume;
   FormGwrap.LabelTitle1.caption:='      '+txt_status+'      ';
   FormGwrap.PanelTitlePLTabAlign.Width:=getalignw(4);
   FormGwrap.ShapeProgress.Color:=PYELLOW;
   FormGwrap.ShapeGlobalProgress.Color:=PYELLOW;
   end;
end;

procedure TFormGwrap.ButtonPauseClick(Sender: TObject);
begin
toggle_pause;
end;
...
{$IFDEF MSWINDOWS}
try
   if Succeeded(ShGetSpecialFolderLocation(FormGwrap.Handle,26,pidl)) then //26 is CSIDL_APPDATA numerical value
...
gocancelall:=true;
setbuttonsnormal;
FormGwrap.LabelKeep.Visible:=false;
FormGwrap.LabelKeep.Caption:='';
FormGwrap.LabelKeep.Hint:='';
FormGwrap.LabelWarning.Visible:=false;
FormGwrap.LabelSpacer.visible:=false;
pcapt:='';
FormGwrap.LabelPerc.Caption:='';
if pprogn<>'' then
   begin
   FormGwrap.ShapeGlobalProgress.visible:=true;
   FormGwrap.ShapeGlobalProgress.height:=pbarhsmall;
   FormGwrap.ShapeProgress.height:=pbarhsmall;
   end
else
   begin
   FormGwrap.ShapeGlobalProgress.visible:=false;
   FormGwrap.ShapeProgress.height:=pbarh;
   end;
iperc:=1;
if exbackground=false then
   if (pprogfirst=true) or (pprogn='') or (pgook=true) then FormGwrap.Visible:=True;
pgook:=false;
//get job type
//0 archive/extract
//1 test;
...
if okseven=true then
   try
   FormGwrap.FTaskBarList.SetProgressState(FormGwrap.AppHandle, TBPF_NOPROGRESS);
   except
   end;
{$ENDIF}
FormGwrap.LabelInfo1.Caption:='--';
FormGwrap.LabelInfo2.Caption:='--';
...
procedure TFormGwrap.FormCreate(Sender: TObject);
begin
launched:=true;
needinteraction:=false;
exbackground:=false;
...
procedure TFormGwrap.l2Click(Sender: TObject);
begin
explore_in;
end;

procedure TFormGwrap.l4Click(Sender: TObject);
begin
explore_out('');
end;

procedure TFormGwrap.Label4Click(Sender: TObject);
var
   P:TProcessUTF8;
...
{$IFDEF MSWINDOWS}
if winver='nt6+' then
   shellexecutew(FormGwrap.handle, PWideChar('find'), PWideChar(''), PWideChar(''), PWideChar (''), SW_SHOWNORMAL)
else
...
procedure TFormGwrap.labelopenfile0Click(Sender: TObject);
begin
cp_search(desk_env);
end;

procedure TFormGwrap.LabelTitle1Click(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle1MouseEnter(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle1MouseLeave(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle2Click(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle2MouseEnter(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle2MouseLeave(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle3Click(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle3MouseEnter(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle3MouseLeave(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle4Click(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle4MouseEnter(Sender: TObject);
begin
...
procedure TFormGwrap.LabelTitle4MouseLeave(Sender: TObject);
begin
...
procedure TFormGwrap.LabelWarningClick(Sender: TObject);
var s:ansistring;
begin
if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_launcher(LabelTitle2,ShapeTitleb2) else clicklabel_launcher(LabelTitle2,Shapelink2);
...
procedure TFormGwrap.LabelDeleteInputClick(Sender: TObject);
var
   p:TPoint;
begin
p.x:=LabelDeleteInput.Left;
p.y:=PageOpt4.top+LabelDeleteInput.top+LabelDeleteInput.Height;
p:=clienttoscreen(p);
PopupMenuDelete.popup(p.x,p.y);
end;

procedure TFormGwrap.MenuItemRecycleClick(Sender: TObject);
begin
erase_fromlauncher(3);
end;

procedure TFormGwrap.MenuItemQuickdeleteClick(Sender: TObject);
begin
erase_fromlauncher(0);
end;

procedure TFormGwrap.MenuItemZerodeleteClick(Sender: TObject);
begin
erase_fromlauncher(2);
end;

procedure TFormGwrap.MenuItemSecuredeleteClick(Sender: TObject);
begin
erase_fromlauncher(1);
end;

procedure TFormGwrap.pm2cancelallClick(Sender: TObject);
begin
exbackground:=false;
TrayIconTask.visible:=false;
gostopall;
...
procedure TFormGwrap.pm2cancelClick(Sender: TObject);
begin
exbackground:=false;
TrayIconTask.visible:=false;
stopped:=true;
...
procedure TFormGwrap.pm2eiClick(Sender: TObject);
begin
explore_in;
end;

procedure TFormGwrap.pm2eoClick(Sender: TObject);
begin
explore_out('');
end;

procedure TFormGwrap.pm2etClick(Sender: TObject);
begin
...
{$IFDEF MSWINDOWS}
if winver<>'nt6+' then
   ShellExecuteW(FormGwrap.Handle, PWideChar  ('open'), PWideChar ('Explorer'), PWideChar  (''), PWideChar  (''), SW_SHOWNORMAL)
else
   ShellExecuteW(FormGwrap.Handle, PWideChar  ('open'), PWideChar ('Explorer'), PWideChar  ('/E,::{20D04FE0-3AEA-1069-A2D8-08002B30309D}'), PWideChar  (''), SW_SHOWNORMAL);
{$ELSE}
cp_open('/',desk_env);
{$ENDIF}
end;

procedure TFormGwrap.pm2exploreClick(Sender: TObject);
begin
do_explorepath;
end;

procedure TFormGwrap.pm2pauseClick(Sender: TObject);
begin
toggle_pause;
end;

procedure TFormGwrap.pm2restoreClick(Sender: TObject);
begin
exbackground:=false;
TrayIconTask.visible:=false;
FormGwrap.visible:=true;
...
procedure TFormGwrap.pm2searchClick(Sender: TObject);
begin
cp_search(desk_env);
end;

procedure TFormGwrap.pmbackgroundClick(Sender: TObject);
begin
exbackground:=true;
TrayIconTask.visible:=true;
FormGwrap.visible:=false;
end;
...
procedure TFormGwrap.pmeoClick(Sender: TObject);
begin
explore_out('');
end;

procedure TFormGwrap.pmetClick(Sender: TObject);
begin
explore_out(poutnamet);
end;

procedure TFormGwrap.pmexploreClick(Sender: TObject);
begin
do_explorepath;
end;
...
procedure TFormGwrap.pmgnometmClick(Sender: TObject);
begin
linlaunch('gnome-system-monitor');
end;

procedure TFormGwrap.pmkdetmClick(Sender: TObject);
begin
linlaunch('KSysGuard');
end;

procedure TFormGwrap.pmmactmClick(Sender: TObject);
begin
linlaunch('open "/System/Applications/Utilities/Activity Monitor.app"');
end;

procedure TFormGwrap.pmsearchClick(Sender: TObject);
begin
cp_search(desk_env);
end;

procedure TFormGwrap.pmtopClick(Sender: TObject);
begin
linlaunchconsole({$IFDEF DARWIN}'/usr/bin/top'{$ELSE}'top'{$ENDIF});
end;

procedure TFormGwrap.pmwintmClick(Sender: TObject);
begin
{$IFDEF MSWINDOWS}
ShellExecuteW(FormGwrap.Handle, PWideChar ('open'), PWideChar('taskmgr'), PWideChar (''), PWideChar (''), SW_SHOWNORMAL);
{$ENDIF}
end;
