unit pea; //Main form of pea executable, providing GUI to file tools, pea/unpea and split/join tasks
{
 ...
 1.27     20251014  G.Tani      Fixed case of checksum digest
...
{$IFDEF DARWIN}MacOSAll, CocoaAll,{$ENDIF}
  ...
  mem_util, list_utils, img_utils, pea_utils, rfs_utils, UnitReport, types;

type

  { TFormPea }

  TFormPea = class(TForm)
    BevelPW: TBevel;
    BevelSplit: TBevel;
    BevelPea: TBevel;
    ButtonClosePea: TBitBtn;
    ButtonDecryptCancel: TBitBtn;
    ButtonPWOK: TBitBtn;
    ButtonPWCancel: TBitBtn;
    ButtonEncryptCancel: TBitBtn;
    ButtonRefSize: TButton;
    ButtonToolsOK: TBitBtn;
    ...
    ComboBoxSplitPresets: TComboBox;
    ComboBoxSplitSizeunit: TComboBox;
    ComboSplitControlalgo: TComboBox;
    ComboBoxUnits: TComboBox;
    ComboBoxUtils: TComboBox;
    EditConfirm: TEdit;
    EditPW: TEdit;
    ImageListPea_dark: TImageList;
    ImageUtils: TImage;
    ImageInfoTools: TImage;
    ImageInfoEncrypt: TImage;
    ImageInfoDecrypt: TImage;
    ImageInfoPW: TImage;
    ...
    LabelConfirm: TLabel;
    LabelDecryptInput: TLabel;
    LabelDecryptOutput: TLabel;
    ...
    LabelEncryptResult1: TLabel;
    LabelEncryptResult2: TLabel;
    LabelHintPW: TLabel;
    LabelKeyFile: TLabel;
    LabelLogPea: TBitBtn;
    LabelOpen: TBitBtn;
    ...
    LabelToolsElapsed: TLabel;
    LabelUtilsFun: TLabel;
    LabelEnt: TLabel;
    LabelSampleEnt: TLabel;
    LabelOpStatus: TLabel;
    LabelToolsResult1: TLabel;
    LabelToolsResult2: TLabel;
    LabelToolsInput: TLabel;
    LabelUtilsInput: TLabel;
    ListMemoUtilsInput: TMemo;
    MainMenu1: TMainMenu;
    ...
    procedure ComboBoxSplitPresetsChange(Sender: TObject);
    ...
    procedure LabelOpenfileSearchClick(Sender: TObject);
    procedure LabelOpenfileSelectClick(Sender: TObject);
    ...
    procedure PanelPWMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
- W.Ehrhardt's hash and crypto libraries are used for hashes, checksums, ciphers
  and key scheduling (PBKDF2, scrypt);
...
var
  ...
  tcapt:ansistring;
begin
tcapt:=FormPea.Caption;
FormPea.Caption:='RUNNING KDF';
...
case upcase(algo) of
   'TRIATS': test_pea_error('creating stream crypto subheader with '+algo,pea_eax256_subhdrP (cxe,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,hdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRITSA': test_pea_error('creating stream crypto subheader with '+algo,pea_tfeax256_subhdrP (cxf,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,fhdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRISAT': test_pea_error('creating stream crypto subheader with '+algo,pea_speax256_subhdrP (cxs,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,shdr256,sbuf1,num_res,niter,'pbkdf2'));
...
case upcase(algo) of
   'TRIATS': test_pea_error('creating stream crypto subheader with '+algo,pea_tfeax256_subhdrP (cxf,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,fhdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRITSA': test_pea_error('creating stream crypto subheader with '+algo,pea_speax256_subhdrP (cxs,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,shdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRISAT': test_pea_error('creating stream crypto subheader with '+algo,pea_eax256_subhdrP (cxe,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,hdr256,sbuf1,num_res,niter,'pbkdf2'));
...
case upcase(algo) of
   'TRIATS': test_pea_error('creating stream crypto subheader with '+algo,pea_speax256_subhdrP (cxs,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,shdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRITSA': test_pea_error('creating stream crypto subheader with '+algo,pea_eax256_subhdrP (cxe,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,hdr256,sbuf1,num_res,niter,'pbkdf2'));
   'TRISAT': test_pea_error('creating stream crypto subheader with '+algo,pea_tfeax256_subhdrP (cxf,persistent_source,fingerprint,ment,kent,fent,7,sbuf2,pw_len,fhdr256,sbuf1,num_res,niter,'pbkdf2'));
...
if ch_number_expected>0 then
   if (upcase(compr)<>'PCOMPRESS0') then
      if ch_size<>1024*1024*1024*1024*1024-volume_authsize then FormPea.LabelEncryptResult1.Caption:='Volume number and total output size may vary due to the compressibility of the input; volume size: '+inttostr(ch_size+volume_authsize)+' B'
      else FormPea.LabelEncryptResult1.Caption:='Expected a single volume archive, output size may vary due to the compressibility of the input'
   else
      if ch_size<>1024*1024*1024*1024*1024-volume_authsize then FormPea.LabelEncryptResult1.Caption:='Expected '+inttostr(ch_number_expected)+' volume(s) of '+inttostr(ch_size+volume_authsize)+' B for a total output size of '+inttostr(exp_size)+' B'
      else FormPea.LabelEncryptResult1.Caption:='Expected a single volume archive of '+inttostr(exp_size)+' B of size'
else FormPea.LabelEncryptResult1.Caption:='Unknown number of volumes expected';
end;
...
var
   ...
   s:ansistring;
begin
FormReport.InputT.Caption:='Input';
FormReport.OutputT.Caption:='Output';
FormReport.Caption:='Log PEA';
FormReport.StringGridInput.ColCount:=7;
FormReport.StringGridInput.Cells[0,0]:='Original object name';
FormReport.StringGridInput.Cells[1,0]:='Status';
FormReport.StringGridInput.Cells[2,0]:='Size (B)';
FormReport.StringGridInput.Cells[3,0]:='Age';
FormReport.StringGridInput.Cells[4,0]:='Attrib';
FormReport.StringGridInput.Cells[5,0]:='Attrib n.';
FormReport.StringGridInput.Cells[6,0]:=obj_algo;
FormReport.StringGridInput.RowCount:=n_input_files+1;
obj_ok:=0;
for k:=0 to n_input_files-1 do
    begin
    FormReport.StringGridInput.Cells[0,k+1]:=in_files[k];
    if status_files[k]=true then FormReport.StringGridInput.Cells[1,k+1]:='Archived'
    else
       begin
       inskipped:=true;
       FormReport.StringGridInput.Cells[1,k+1]:='Skipped';
       end;
    if status_files[k]=true then
       begin
       FormReport.StringGridInput.Cells[2,k+1]:=inttostr(fsizes[k]);
       if ftimes[k]<>0 then FormReport.StringGridInput.Cells[3,k+1]:=datetimetostr(filedatetodatetime(ftimes[k]));
       FormReport.StringGridInput.Cells[4,k+1]:=fattr_dec[k];
       FormReport.StringGridInput.Cells[5,k+1]:=inttostr(fattr[k]);
       if upcase(obj_algo)<>'NOALGO' then
          begin
          s:='';
          for h:=0 to obj_authsize-1 do s:=s+hexstr(@obj_tags[k,h],1);
          FormReport.StringGridInput.Cells[6,k+1]:=s;
          end;
       inc(obj_ok,1);
       end;
    end;
FormReport.StringGridInput.AutosizeColumns;
FormReport.StringGridOutput.ColCount:=2;
FormReport.StringGridOutput.Cells[0,0]:='Volume';
FormReport.StringGridOutput.Cells[1,0]:=volume_algo;
FormReport.StringGridOutput.RowCount:=j+1;
for k:=0 to j-1 do
    begin
    update_pea_filename(out_path+out_name,k+1,s);
    FormReport.StringGridOutput.Cells[0,k+1]:=s;
    if upcase(volume_algo)<>'NOALGO' then
       begin
       s:='';
       for h:=0 to volume_authsize-1 do s:=s+hexstr(@volume_tags[k,h],1);
       FormReport.StringGridOutput.Cells[1,k+1]:=s;
       end;
    end;
...
//output name
FormReport.LabelReport4.Caption:=FormPea.LabelEncryptOutput.Caption;
end;
...
if (upcase(pw_param)<>'HIDDEN') and (upcase(pw_param)<>'HIDDEN_REPORT') then FormPea.Visible:=true else FormPea.Visible:=false;
FormPea.Caption:='Pea';
FormPea.PanelDecrypt.visible:=false;
FormPea.PanelEncrypt.visible:=true;
...
FormPea.ButtonClosePea.Visible:=true;
FormPea.ButtonEncryptCancel.Visible:=false;
Application.ProcessMessages;
...
Sleep(500);
if closepolicy>0 then FormPea.Close; //error conditions are intercepted before and handled with internal_error procedure
end;
...
if afn[length(afn)]=DirectorySeparator then
   begin
   extractdirname(afn,fnpath,fnname);
   if subroot='' then
      begin
      subroot:=fnpath;
      basedir:=afn;
...
hdrd256:=hdr256;
if FCA_EAX256_initP(cxe, @sbuf2, pw_len, hdrd256, niter, 'pbkdf2')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
...
if FCf_EAX256_initP(cxf, @sbuf2, pw_len, fhdrd256, niter, 'pbkdf2')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
...
if FCs_EAX256_initP(cxs, @sbuf2, pw_len, shdrd256, niter, 'pbkdf2')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
...
hdr256.PW_Ver:=hdr.PW_Ver;
hdrd256:=hdr256;
if FCA_EAX256_initP(cxe, @sbuf2, pw_len, hdrd256, niter, 'scrypt')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
fhdr256.FCfsig:=fhdr.FCfsig;
fhdr256.Flags:=fhdr.Flags;
fhdr256.Salt:=fhdr.Salt;
fhdr256.PW_Ver:=fhdr.PW_Ver;
fhdrd256:=fhdr256;
for i:=0 to pw_len-1 do sbuf2[i]:=tsbuf2[i] xor (pw_len+i) xor ord(upcase(algo[length(algo)-1]));
if FCf_EAX256_initP(cxf, @sbuf2, pw_len, fhdrd256, niter, 'scrypt')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
shdr256.FCssig:=shdr.FCssig;
shdr256.Flags:=shdr.Flags;
shdr256.Salt:=shdr.Salt;
shdr256.PW_Ver:=shdr.PW_Ver;
...
for i:=0 to pw_len-1 do sbuf2[i]:=tsbuf2[i];
for i:=0 to pw_len-1 do sbuf2[i]:=sbuf2[i] xor (pw_len xor i) xor ord(upcase(algo[length(algo)]));
if FCs_EAX256_initP(cxs, @sbuf2, pw_len, shdrd256, niter, 'scrypt')<>0 then internal_error('Stream control algorithm: error in '+algo+' init');
verw:=hdrd256.PW_Ver xor fhdrd256.PW_Ver xor shdrd256.PW_Ver;
if shdr256.PW_ver<>verw then internal_error('Wrong password or keyfile');
for i:=0 to pw_len-1 do tsbuf2[i]:=0;
verw:=0;
...
if upcase(algo)<>'NOALGO' then
   ...
   if tag_match=false then
      ...
      (upcase(algo)='EAX') or (upcase(algo)='TF') or (upcase(algo)='SP') or (upcase(algo)='HMAC') then FormPea.LabelDecryptResult1.Caption:='Archive''s stream correctly authenticated, tag: '+s
      else FormPea.LabelDecryptResult1.Caption:='Archive''s stream correctly verified';
      end;
   end;
end;
...
FormReport.InputT.Caption:='Objects';
FormReport.OutputT.Caption:='Volumes';
FormReport.Caption:='Log UnPEA';
FormReport.StringGridInput.ColCount:=8;
FormReport.StringGridInput.Cells[0,0]:='Original object name';
FormReport.StringGridInput.Cells[1,0]:='Status';
FormReport.StringGridInput.Cells[2,0]:='Size (B)';
FormReport.StringGridInput.Cells[3,0]:='Age';
FormReport.StringGridInput.Cells[4,0]:='Attrib';
FormReport.StringGridInput.Cells[5,0]:='Attrib n.';
FormReport.StringGridInput.Cells[6,0]:='calculated ('+obj_algo+')';
FormReport.StringGridInput.Cells[7,0]:='found';
FormReport.StringGridInput.RowCount:=nobj+2;
obj_ok:=0;
for k:=0 to nobj do
    begin
    FormReport.StringGridInput.Cells[0,k+1]:=ansitoutf8(in_files[k]);
    FormReport.StringGridInput.Cells[1,k+1]:=status_objects[k];
    FormReport.StringGridInput.Cells[2,k+1]:=inttostr(fsizes[k]);
    if system_datetimeencoding=archive_datetimeencoding then
       ...
       try
          ...
          FormReport.StringGridInput.Cells[3,k+1]:='Non valid DateTime';
       end;
       end
    else FormReport.StringGridInput.Cells[3,k+1]:='DateTime conversion not available';
    FormReport.StringGridInput.Cells[4,k+1]:=fattr_dec[k];
    FormReport.StringGridInput.Cells[5,k+1]:=inttostr(fattr[k]);
    if upcase(obj_algo)<>'NOALGO' then
       begin
       ...
       FormReport.StringGridInput.Cells[7,k+1]:=s;
       end;
    inc(obj_ok,1);
    end;
FormReport.StringGridInput.AutosizeColumns;
FormReport.StringGridOutput.ColCount:=4;
FormReport.StringGridOutput.Cells[0,0]:='Volume';
FormReport.StringGridOutput.Cells[1,0]:='Status';
FormReport.StringGridOutput.Cells[2,0]:='calculated ('+volume_algo+')';
FormReport.StringGridOutput.Cells[3,0]:='found';
FormReport.StringGridOutput.RowCount:=j;
for k:=0 to j-2 do
    begin
    ...
    if upcase(volume_algo)<>'NOALGO' then
       begin
       FormReport.StringGridOutput.Cells[1,k+1]:=status_volumes[k];
       s:='';
       for h:=0 to volume_authsize-1 do s:=s+hexstr(@volume_tags[k,h],1);
       FormReport.StringGridOutput.Cells[2,k+1]:=s;
       s:='';
       for h:=0 to volume_authsize-1 do s:=s+hexstr(@exp_volume_tags[k,h],1);
       FormReport.StringGridOutput.Cells[3,k+1]:=s;
       end;
    end;
FormReport.StringGridOutput.AutosizeColumns;
...
clean_variables;
if (upcase(pw_param)<>'HIDDEN') and (upcase(pw_param)<>'HIDDEN_REPORT') then FormPea.Visible:=true else FormPea.Visible:=false;
FormPea.PanelDecrypt.visible:=true;
FormPea.PanelEncrypt.visible:=false;
FormPea.Caption:='UnPea';
ts_start:=datetimetotimestamp(now);
stream_error:=false;
obj_error:=false;
volume_error:=false;
...
if ch_size<>1024*1024*1024*1024*1024 then FormPea.LabelEncryptResult1.Caption:='Expected '+inttostr(ch_number_expected)+' volume(s) of '+inttostr(ch_size+volume_authsize)+' B for a total output size of '+inttostr(exp_size)+' B'
else FormPea.LabelEncryptResult1.Caption:='Expected a single volume of '+inttostr(exp_size)+' B of size';
end;
...
FormReport.InputT.Caption:='Input';
FormReport.OutputT.Caption:='Output';
FormReport.Caption:='Split file log';
FormReport.StringGridInput.ColCount:=3;
FormReport.StringGridInput.Cells[0,0]:='Original object name';
FormReport.StringGridInput.Cells[1,0]:='Status';
FormReport.StringGridInput.Cells[2,0]:='Size (B)';
FormReport.StringGridInput.RowCount:=2;
FormReport.StringGridInput.Cells[0,1]:=in_qualified_name;
FormReport.StringGridInput.Cells[1,1]:='OK';
FormReport.StringGridInput.Cells[2,1]:=inttostr(file_size);
FormReport.StringGridInput.AutosizeColumns;
FormReport.StringGridOutput.ColCount:=2;
FormReport.StringGridOutput.Cells[0,0]:='Volume';
FormReport.StringGridOutput.Cells[1,0]:=volume_algo;
...
for k:=0 to j-1 do
    begin
    FormReport.StringGridOutput.Cells[0,k+1]:=inttostr(k+1);
    if upcase(volume_algo)<>'NOALGO' then
       begin
       s:='';
       for h:=0 to volume_authsize-1 do s:=s+hexstr(@volume_tags[k,h],1);
...
do_report_rfs;
FormPea.LabelEncryptResult1.Caption:=FormReport.LabelReport2.Caption;
FormPea.LabelEncryptSettings.Visible:=true;
FormPea.LabelEncryptResult1.Visible:=true;
FormPea.LabelEncryptResult2.Visible:=true;
end;
...
var
   ...
   s:ansistring;
begin
FormReport.InputT.Caption:='File';
FormReport.OutputT.Caption:='Volumes';
FormReport.Caption:='Log Raw File Join';
FormReport.StringGridInput.ColCount:=2;
FormReport.StringGridInput.Cells[0,0]:='File name';
FormReport.StringGridInput.Cells[1,0]:='Size (B)';
FormReport.StringGridInput.RowCount:=2;
FormReport.StringGridInput.Cells[0,1]:=out_param;
FormReport.StringGridInput.Cells[1,1]:=inttostr(exp_space);
FormReport.StringGridInput.AutosizeColumns;
FormReport.StringGridOutput.ColCount:=4;
FormReport.StringGridOutput.Cells[0,0]:='Volume';
FormReport.StringGridOutput.Cells[1,0]:='Status';
FormReport.StringGridOutput.Cells[2,0]:='calculated ('+volume_algo+')';
FormReport.StringGridOutput.Cells[3,0]:='found';
if j>1 then FormReport.StringGridOutput.RowCount:=j
else exit;
for k:=0 to j-2 do
    begin
    FormReport.StringGridOutput.Cells[0,k+1]:=inttostr(k+1);
...
FormPea.LabelDecryptInfo.Caption:='Integrity check algorithm: '+volume_algo;
{
...
do_report_rfj;
timing(ts_start,wrk_space);
FormPea.LabelOpen.Caption:='Open';
output:=out_path+out_file;
FormPea.LabelOpen.visible:=true;
FormPea.LabelLogPea.Visible:=true;
FormPea.ButtonClosePea.Visible:=true;
...
begin
FormPea.LabelToolsResult2.Caption:='Operation cancelled by user';
Application.ProcessMessages;
if errors=0 then
   begin
   Sleep(1500);
...
repeat
   ...
   FormPea.ProgressBarPea.Position:=(100*etsize) div tsize;
   tsout:=datetimetotimestamp(now);
   time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
   FormPea.LabelToolsElapsed.Caption:=nicetime(inttostr(time))+' elapsed';
   Application.ProcessMessages;
...
FormPea.PanelPW.height:=2;
FormPea.ImageInfoTools.Picture.Bitmap:=Binfo;
FormPea.ButtonToolsCancel.visible:=true;
FormPea.ButtonToolsCancel.hint:='Cancel will stop deletion but will not recover already deleted elements';
FormReport.NotebookReport.PageIndex:=0;
FormReport.StringGridInput.RowCount:=1;
level:=upcase(level);
if (level='NONE') or (level='QUICK') or (level='RECYCLE') or (level='HEADER') then
   FormReport.Caption:='Delete'
else
   FormReport.Caption:='Secure delete';
FormPea.Caption:=FormReport.Caption;
FormPea.LabelToolsInput.Caption:=FormReport.Caption+' ('+level+'), '+inttostr(paramcount-2)+' element(s)';
FormPea.ProgressBarPea.Position:=0;
FormPea.PanelTools.Cursor:=crHourGlass;
errors:=0;
dfiles:=0;
ddirs:=0;
...
FormPea.LabelToolsInput.Caption:=FormPea.LabelToolsInput.Caption+', '+nicenumber(inttostr(tsize),0);
end2caption:=FormPea.LabelToolsInput.Caption;
Application.ProcessMessages;
tsize:=(tsize*nlevel) + paramcount;
randomize;
for j:=3 to paramcount do
   begin
   if FormPea.ProgressBarPea.Position>=100 then FormPea.ProgressBarPea.Position:=99;
   tsout:=datetimetotimestamp(now);
   time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
   FormPea.LabelToolsResult2.Caption:=nicetime(inttostr(time))+' elapsed';
   Application.ProcessMessages;
   try
      ...
      for k:=0 to nfound-1 do
         ...
         if filegetattr(exp_files[k]) and faDirectory = 0 then //file
            begin
            rc:=FormReport.StringGridInput.RowCount+1;
            FormReport.StringGridInput.RowCount:=rc;
            if nlevel>0 then FormPea.LabelToolsInput.Caption:=end2caption+', '+nicenumber(inttostr(etsize div nlevel),0)+' deleted'
            else FormPea.LabelToolsInput.Caption:=end2caption;
            FormPea.LabelToolsResult1.Caption:='Processing item '+inttostr(rc-1)+' of '+inttostr(ntotalexp)+' found';
            application.ProcessMessages;
            try
               {$IFDEF MSWINDOWS}
               ...
               closefile(f);
               FormPea.LabelToolsResult1.Caption:=FormPea.LabelToolsResult1.Caption+', '+nicenumber(inttostr(size),0)+' file';
               ...
               FormReport.StringGridInput.Cells[0,rc-1]:=exp_files[k];
               FormReport.StringGridInput.Cells[1,rc-1]:='file successfully deleted';
               dfiles:=dfiles+1;
            except
               try closefile(f); except end;
               FormReport.StringGridInput.Cells[0,rc-1]:=exp_files[k];
               FormReport.StringGridInput.Cells[1,rc-1]:='<ERROR: FILE NOT DELETED (not writeable/accessible/found)>';
               errors:=errors+1;
            end;
            end
         else
            ...
            if not(fileexists(exp_files[k])) and not(directoryexists(exp_files[k])) then //not found
               ...
               FormReport.StringGridInput.RowCount:=rc;
               FormReport.StringGridInput.Cells[0,rc-1]:=exp_files[k];
               FormReport.StringGridInput.Cells[1,rc-1]:='OBJECT NOT FOUND';
               errors:=errors+1;
               end
            else
               begin
      ...
      if length(exp_dirs)>0 then
         for k:=(length(exp_dirs)-1) downto 0 do
            ...
            rc:=FormReport.StringGridInput.RowCount+1;
            FormReport.StringGridInput.RowCount:=rc;
            FormPea.LabelToolsResult1.Caption:='Processing item '+inttostr(rc-1)+' of '+inttostr(ntotalexp)+', directory';
            Application.ProcessMessages;
            try
               ...
               FormReport.StringGridInput.Cells[0,rc-1]:=exp_dirs[k];
               FormReport.StringGridInput.Cells[1,rc-1]:='directory successfully deleted';
               ddirs:=ddirs+1;
            except
               FormReport.StringGridInput.Cells[0,rc-1]:=exp_dirs[k];
               FormReport.StringGridInput.Cells[1,rc-1]:='DIRECTORY NOT DELETED (not writeable/accessible/found)';
               errors:=errors+1;
            end;
            end;
      end;
       ...
       rc:=FormReport.StringGridInput.RowCount+1;
       FormReport.StringGridInput.RowCount:=rc;
       FormReport.StringGridInput.Cells[0,FormReport.StringGridInput.RowCount-1]:=(paramstr(j));
       FormReport.StringGridInput.Cells[1,FormReport.StringGridInput.RowCount-1]:='OBJECT NOT DELETED';
       errors:=errors+1;
   end;
   end;
FormPea.ButtonToolsCancel.visible:=false;
FormPea.ProgressBarPea.Position:=100;
FormPea.PanelTools.Cursor:=crDefault;
FormReport.StringGridInput.Cells[0,0]:='File';
FormReport.StringGridInput.Cells[1,0]:='Result';
FormReport.StringGridInput.AutosizeColumns;
FormPea.LabelToolsInput.Caption:=end2caption;
if level='RECYCLE' then
   FormPea.LabelToolsResult1.Caption:=''
else
   FormPea.LabelToolsResult1.Caption:='Processed '+inttostr(dfiles)+' files, '+inttostr(ddirs)+' directories, '+inttostr(errors)+' errors';
tsout:=datetimetotimestamp(now);
time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
if time>0 then
   begin
   speed:=(tsize * 1000) div time;
   FormPea.LabelToolsResult2.Caption:=nicetime(inttostr(time))+' total time @ '+nicenumber(inttostr(speed),0)+'/s';
   end
else FormPea.LabelToolsResult2.Caption:='';
FormReport.LabelReport1.Caption:=FormPea.LabelToolsInput.Caption;
FormReport.LabelReport2.Caption:=FormPea.LabelToolsResult1.Caption;
FormReport.LabelReport3.Caption:=FormPea.LabelToolsResult2.Caption;
...
if errors=0 then
   begin
   if level<>'RECYCLE' then FormPea.ImageInfoTools.Picture.Bitmap:=Bok;
   Application.ProcessMessages;
   exitcode:=0;
   Sleep(1500);
   if closepolicy>0 then FormPea.Close;
   ...
   begin
   FormPea.ImageInfoTools.Picture.Bitmap:=Bcancel;
   Application.ProcessMessages;
   exitcode:=-2;
   end;
end;
...
begin
FormPea.LabelToolsResult2.Caption:=nicetime(inttostr(time))+' operation cancelled by user, recovering free space from '+wrkdir+' ...';
Application.ProcessMessages;
recoverfreespace(n);
removedir(wrkdir);
sleep(1500);
...
begin
   ...
   repeat
      ...
      repeat
         ...
         FormPea.ProgressBarPea.Position:=100-((rfree*100) div sizefree);
         FormPea.LabelToolsResult1.Caption:=nicenumber(inttostr(sizefree),0)+' free, '+nicenumber(inttostr(rfree),0)+' remaining';
         tsout:=datetimetotimestamp(now);
         time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
         FormPea.LabelToolsResult2.Caption:=nicetime(inttostr(time))+' elapsed';
         Application.ProcessMessages;
   ...
   until tok=true;
   recoverfreespace(n);
   FormPea.LabelToolsInput.Caption:='Done drive '+(paramstr(3))+', '+nicenumber(inttostr(sizetotal),0)+', pass '+inttostr(j)+' of '+inttostr(nlevel);
   end;
...
FormReport.Caption:=wrktitle;
FormPea.Caption:=wrktitle;
FormPea.ProgressBarPea.Position:=0;
FormPea.PanelTools.Cursor:=crHourGlass;
case level of
   'ZERO' : nlevel:=1;
...
if (winver='nt6+') or (winver='nt5') then
   ...
   if fstype<>'' then
      FormPea.LabelToolsInput.Caption:='Processing drive '+(paramstr(3))+' ('+fstype+'), '+nicenumber(inttostr(sizetotal),0)
   else
      FormPea.LabelToolsInput.Caption:='Processing drive '+(paramstr(3))+', '+nicenumber(inttostr(sizetotal),0);
Application.ProcessMessages;
case level of
   'ZERO': //overwrite with zero
   sanitizefixed(0,1);
   ...
   if fstype<>'' then
      FormPea.LabelToolsInput.Caption:='Processing drive '+(paramstr(3))+' ('+fstype+'), '+nicenumber(inttostr(sizetotal),0)+', pass '+inttostr(j+nlevel-nleveli)+' of '+inttostr(nlevel)
   else
      FormPea.LabelToolsInput.Caption:='Processing drive '+(paramstr(3))+', '+nicenumber(inttostr(sizetotal),0)+', pass '+inttostr(j+nlevel-nleveli)+' of '+inttostr(nlevel);
   Application.ProcessMessages;
   ...
   until tok=true;
   recoverfreespace(n);
   FormPea.LabelToolsInput.Caption:='Done drive '+(paramstr(3))+', '+nicenumber(inttostr(sizetotal),0)+', pass '+inttostr(j+nlevel-nleveli)+' of '+inttostr(nlevel);
   end;
   end;
end;
FormPea.ButtonToolsCancel.visible:=false;
...
FormReport.StringGridInput.AutosizeColumns;
rfree:=diskfree(drivenumber);
FormPea.LabelToolsResult1.Caption:=nicenumber(inttostr(sizefree),0)+' free when task started, '+nicenumber(inttostr(rfree),0)+' currently free, temporary work dir '+wrkdir;
tsout:=datetimetotimestamp(now);
time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
if time>0 then
   begin
...
FormReport.LabelReport1.Caption:=wrktitle;
FormReport.LabelReport2.Caption:=FormPea.LabelToolsInput.Caption;
FormReport.LabelReport3.Caption:=FormPea.LabelToolsResult1.Caption;
FormReport.LabelReport4.Caption:=FormPea.LabelToolsResult2.Caption;
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormPea.ButtonClosePea.Visible:=true;
FormPea.LabelOpen.Visible:=true;
FormPea.LabelOpen.Enabled:=false;
...
FormReport.NotebookReport.PageIndex:=0;
FormReport.StringGridInput.RowCount:=1;
FormReport.Caption:='Compare';
FormPea.Caption:='Byte to byte compare';
FormPea.LabelToolsInput.Caption:='Comparing files...';
FormPea.LabelToolsResult1.Caption:='First file: '+(paramstr(2));
sfile:=paramstr(3);
if sfile='' then
   if FormPea.OpenDialogInput.Execute then
      if FormPea.OpenDialogInput.FileName<>'' then sfile:=FormPea.OpenDialogInput.FileName
      else exit;
FormPea.LabelToolsResult2.Caption:='Second file: '+(sfile);
FormPea.ProgressBarPea.Position:=0;
FormReport.StringGridInput.RowCount:=2;
FormReport.StringGridInput.Cells[0,0]:='Test';
FormReport.StringGridInput.Cells[1,0]:='Result';
continue:=0;
total:=0;
d:=0;
...
if total<>0 then
   begin
   FormReport.StringGridInput.RowCount:=FormReport.StringGridInput.RowCount+1;
   FormReport.StringGridInput.Cells[0,1]:='Error';
   FormReport.StringGridInput.Cells[1,1]:='Cannot compare files';
   if paramstr(2)=sfile then
      FormPea.LabelToolsInput.Caption:='Cannot compare a file with itself!'
   else
      begin
      FormPea.LabelToolsInput.Caption:='Cannot compare files';
      if total=2 then
         begin
         if sizea=0 then FormPea.LabelToolsInput.Caption:=FormPea.LabelToolsInput.Caption+' first file is empty';
         if sizeb=0 then FormPea.LabelToolsInput.Caption:=FormPea.LabelToolsInput.Caption+' second file is empty';
         end
      else
         FormPea.LabelToolsInput.Caption:=FormPea.LabelToolsInput.Caption+' (i.e. not accessible, not files etc)';
      end;
   FormPea.ProgressBarPea.Position:=100;
   FormReport.StringGridInput.AutosizeColumns;
   FormReport.LabelReport1.Caption:=FormPea.Caption;
   FormReport.LabelReport2.Caption:=FormPea.LabelToolsInput.Caption;
   FormReport.LabelReport3.Caption:=FormPea.LabelToolsResult1.Caption;
   FormReport.LabelReport4.Caption:=FormPea.LabelToolsResult2.Caption;
   {$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
...
if sizea=sizeb then
   begin
   FormReport.StringGridInput.Cells[0,1]:='Size comparison';
   FormReport.StringGridInput.Cells[1,1]:='Files have same size: '+inttostr(sizea)+' B';
   x:=1;
   end
else
   begin
   FormReport.StringGridInput.Cells[0,1]:='Size comparison';
   FormReport.StringGridInput.Cells[1,1]:='Files have different sizes';
   FormReport.StringGridInput.RowCount:=FormReport.StringGridInput.RowCount+3;
   FormReport.StringGridInput.Cells[0,2]:='- First file';
   FormReport.StringGridInput.Cells[1,2]:=inttostr(sizea)+' B';
   FormReport.StringGridInput.Cells[0,3]:='- Second file';
   FormReport.StringGridInput.Cells[1,3]:=inttostr(sizeb)+' B';
   FormReport.StringGridInput.Cells[0,4]:='- Size difference';
   if sizea>sizeb then FormReport.StringGridInput.Cells[1,4]:=inttostr(sizea-sizeb)+' B'
   else FormReport.StringGridInput.Cells[1,4]:=inttostr(sizeb-sizea)+' B';
   x:=4;
   end;
FormReport.LabelReport1.Caption:=FormPea.Caption;
FormReport.LabelReport2.Caption:=FormPea.LabelToolsInput.Caption;
FormReport.LabelReport3.Caption:=FormPea.LabelToolsResult1.Caption+' ('+inttostr(sizea)+' B)';
FormReport.LabelReport4.Caption:=FormPea.LabelToolsResult2.Caption+' ('+inttostr(sizeb)+' B)';
FormPea.Visible:=false;
...
repeat
   ...
   for i:=0 to (numreadmin - 1) do
      ...
      if bufa[i]=bufb[i] then
         ...
         d:=d+1;
         FormReport.StringGridInput.RowCount:=FormReport.StringGridInput.RowCount+1;
         FormReport.StringGridInput.Cells[0,d+x]:='Offset '+system.hexstr(((filepos(fa)-numreada+i)div 16)*16,8);
         FormReport.StringGridInput.Cells[1,d+x]:='Byte '+inttostr(filepos(fa)-numreada+i+1)+' is different: Hex '+hexstr(@bufa[i],1)+' vs '+hexstr(@bufb[i],1)+'; Dec '+inttostr(bufa[i])+' vs '+inttostr(bufb[i]);
         if ((d>=100) and (continue=0)) then
            ...
            if MessageDlg('More than 100 different bytes, continue anyway?',mtConfirmation,[mbYes, mbNo],0)=6 then continue:=1
               ...
               begin
               FormReport.StringGridInput.Cells[0,d+x]:='Comparison terminated by user';
               FormReport.StringGridInput.Cells[1,d+x]:='More than 100 different bytes';
         ...
         if (d>=10000) then
            begin
            FormReport.StringGridInput.Cells[0,d+x]:='Comparison automatically terminated';
            FormReport.StringGridInput.Cells[1,d+x]:='More than 10000 different bytes';
            stoppedcomparison:=true;
            break;
            end;
         end;
   ...
   FormPea.ProgressBarPea.Position:=(total*100) div sizemin;
   Application.ProcessMessages;
...
closefile(fb);
FormReport.StringGridInput.RowCount:=FormReport.StringGridInput.RowCount+1;
FormReport.StringGridInput.Cells[0,FormReport.StringGridInput.RowCount-1]:='Byte comparison';
if d=0 then
   if sizea=sizeb then
      begin
      FormPea.Caption:='Files are identical';
      FormPea.LabelToolsInput.Caption:='Same size, no different byte';
      FormReport.StringGridInput.Cells[1,FormReport.StringGridInput.RowCount-1]:='No different byte';
      end
   else
      begin
      FormPea.Caption:='Files are different';
      FormPea.LabelToolsInput.Caption:='Different size, no different byte in the shortest file ('+inttostr(sizemin)+' B)';
      FormReport.StringGridInput.Cells[1,FormReport.StringGridInput.RowCount-1]:='No different byte in the shortest file ('+inttostr(sizemin)+' B)';
      end
else
   if sizea=sizeb then
      begin
      FormPea.Caption:='Files are different';
      FormPea.LabelToolsInput.Caption:='Same size, '+inttostr(d)+' different byte(s)';
      FormReport.StringGridInput.Cells[1,FormReport.StringGridInput.RowCount-1]:=inttostr(d)+' different byte(s)';
      end
   else
      begin
      FormPea.Caption:='Files are different';
      FormPea.LabelToolsInput.Caption:='Different size, '+inttostr(d)+' different byte(s) in the shortest file ('+inttostr(sizemin)+' B)';
      FormReport.StringGridInput.Cells[1,FormReport.StringGridInput.RowCount-1]:=inttostr(d)+' different byte(s) in the shortest file ('+inttostr(sizemin)+' B)';
      end;
FormPea.ProgressBarPea.Position:=100;
FormReport.StringGridInput.AutosizeColumns;
FormReport.LabelReport1.Caption:=FormPea.Caption;
...
FormPea.PanelPW.height:=2;
FormPea.LabelToolsInput.Caption:='Benchmark started...';
...
FormPea.ImageInfoTools.Hint:=shint;
FormPea.LabelToolsInput.Hint:=shint;
...
if times>0 then
   ...
   FormPea.LabelToolsResult1.Caption:='Single core '+inttostr(points);
   end;
...
if (timem>0) and (times>0) then
   ...
   FormPea.LabelToolsInput.Caption:='Arithmetic benchmark completed';
   FormPea.LabelToolsResult2.Caption:='Multi core '+inttostr(pointm);
   pointratio:=pointm/points;
   FormPea.LabelToolsResult2.Caption:=FormPea.LabelToolsResult2.Caption+' | ratio '+FloatToStrF(pointratio,ffFixed,4,2);
   end
else FormPea.LabelToolsInput.Caption:='Benchmark failed';
...
begin
FormPea.LabelToolsResult2.Caption:='Operation cancelled by user, terminating...';
Application.ProcessMessages;
sleep(1500);
halt(-4);
end;
...
if bmode='MULTI' then FormPea.Caption:='Running benchmark segment '+inttostr(t)+'/'+inttostr(tt)
else FormPea.Caption:='Running benchmark';
biter:=900;
//FormPea.ButtonToolsCancel.visible:=true;
FormPea.LabelToolsInput.Caption:='Benchmark running...';
FormPea.ProgressBarPea.Position:=0;
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
Randomize;
max64 := High(Int64);
for j:=1 to biter do
   ...
   FormPea.ProgressBarPea.Position:=(100*j) div (biter);
   tsout:=datetimetotimestamp(now);
   time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
   FormPea.LabelToolsElapsed.Caption:=nicetime(inttostr(time))+' elapsed';
...
if time>0 then
   begin
   FormPea.LabelToolsInput.Caption:='Benchmark completed';
   FormPea.LabelToolsElapsed.Caption:=nicetime(inttostr(time))+' elapsed (lower, better)';
   end
else FormPea.LabelToolsInput.Caption:='Benchmark failed';
//FormPea.ButtonClosePea.Visible:=true;
exitcode:=0;
...
FormReport.NotebookReport.PageIndex:=0;
specop:='';
FormPea.LabelToolsInput.Caption:='Checking file(s)...';
FormPea.ProgressBarPea.Position:=0;
FormReport.InputT.Caption:='Input';
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormReport.StringGridInput.ColCount:=31;
FormReport.StringGridInput.Cells[0,0]:='File path and name';
FormReport.StringGridInput.Cells[1,0]:='Name';
FormReport.StringGridInput.Cells[2,0]:='Type';
FormReport.StringGridInput.Cells[3,0]:='Size';
FormReport.StringGridInput.Cells[4,0]:='Bytes';
FormReport.StringGridInput.Cells[5,0]:='Modified';
FormReport.StringGridInput.Cells[6,0]:='Attributes';
FormReport.StringGridInput.Cells[7,0]:='Copies';
FormReport.StringGridInput.Cells[8,0]:='Adler32';
FormReport.StringGridInput.Cells[9,0]:='CRC16';
FormReport.StringGridInput.Cells[10,0]:='CRC24';
FormReport.StringGridInput.Cells[11,0]:='CRC32';
FormReport.StringGridInput.Cells[12,0]:='CRC64';
FormReport.StringGridInput.Cells[13,0]:='eDonkey';
FormReport.StringGridInput.Cells[14,0]:='MD4';
FormReport.StringGridInput.Cells[15,0]:='MD5';
FormReport.StringGridInput.Cells[16,0]:='RIPEMD160';
FormReport.StringGridInput.Cells[17,0]:='SHA1';
//FormReport.StringGridInput.Cells[18,0]:='SHA224';
FormReport.StringGridInput.Cells[18,0]:='BLAKE2S';
FormReport.StringGridInput.Cells[19,0]:='SHA256';
FormReport.StringGridInput.Cells[20,0]:='SHA3_256';
//FormReport.StringGridInput.Cells[21,0]:='SHA384';
FormReport.StringGridInput.Cells[21,0]:='BLAKE2B';
FormReport.StringGridInput.Cells[22,0]:='SHA512';
FormReport.StringGridInput.Cells[23,0]:='SHA3_512';
FormReport.StringGridInput.Cells[24,0]:='Whirlpool';
FormReport.StringGridInput.Cells[25,0]:='(Encoded size)';
FormReport.StringGridInput.Cells[26,0]:='File header';
FormReport.StringGridInput.Cells[27,0]:='End of file';
FormReport.StringGridInput.Cells[28,0]:='Directory content';
FormReport.StringGridInput.Cells[29,0]:='% size';
FormReport.StringGridInput.Cells[30,0]:='(Encoded % size)';
...
if oper='CRCHASH' then FormPea.Caption:='Checksum and hash'
else FormPea.Caption:='Analyze';
if (oper='CRCHASH') and (mode<>'BASE64') then FormReport.LabelCase.visible:=true;
FormReport.Caption:=FormPea.Caption;
//get input size
tsize:=0;
etsize:=0;
ctsize:=0;
nfiles:=0;
ntfiles:=0;
FormPea.LabelToolsInput.Caption:='Checking ('+moded+') '+inttostr(paramcount-j+1)+' element(s), counting total items and size...';
Application.ProcessMessages;
...
if oper='CRCHASH' then
   if ntotalexp>1 then FormReport.StringGridInput.Rowcount:=ntotalexp+2
   else FormReport.StringGridInput.Rowcount:=ntotalexp+1
else FormReport.StringGridInput.Rowcount:=ntotalexp+1;
for k:=0 to nfound-1 do
   ...
   FormReport.StringGridInput.Cells[0,t]:=(exp_files[k]);
   FormReport.StringGridInput.Cells[1,t]:=extractfilename((exp_files[k]));
   FormReport.StringGridInput.Cells[2,t]:=extractfileext((exp_files[k]));
   FormReport.StringGridInput.Cells[3,t]:=nicenumber(inttostr(f_size),0);
   FormReport.StringGridInput.Cells[4,t]:=inttostr(f_size);
   FormReport.StringGridInput.Cells[5,t]:=FormatDateTime('yyyy-mm-dd hh:mm:ss', filedatetodatetime(exp_ftimes[k]));
   FormReport.StringGridInput.Cells[6,t]:=exp_fattr_dec[k];
   FormReport.StringGridInput.Cells[25,t]:=inttostr(length(inttostr(length(FormReport.StringGridInput.Cells[4,t]))))+inttostr(length(FormReport.StringGridInput.Cells[4,t]))+FormReport.StringGridInput.Cells[4,t];
   if tsize>0 then FormReport.StringGridInput.Cells[29,t]:=inttostr((100*f_size) div tsize)+'%';
   if tsize>0 then FormReport.StringGridInput.Cells[30,t]:=inttostr(length(FormReport.StringGridInput.Cells[27,t]))+FormReport.StringGridInput.Cells[27,t];
   ...
   FormPea.LabelToolsInput.Caption:='Checking ('+moded+') '+inttostr(paramcount-j+1)+' element(s), '+nicenumber(inttostr(tsize),0)+', '+nicenumber(inttostr(etsize),0)+' checked';
   FormPea.LabelToolsResult1.Caption:='Processing item '+inttostr(t)+' of '+inttostr(ntfiles)+', '+nicenumber(inttostr(f_size),0)+' file';
   Application.ProcessMessages;
   except
   FormReport.StringGridInput.Cells[0,t]:=(exp_files[k]);
   FormReport.StringGridInput.Cells[1,t]:=extractfilename((exp_files[k]));
   FormReport.StringGridInput.Cells[2,t]:=extractfileext((exp_files[k]));
   FormReport.StringGridInput.Cells[3,t]:='ERROR';
   te:=te+1;
   continue;
   end;
   if oper<>'LIST' then
   ...
   repeat
      ...
      if n<>0 then
         ...
         etsize:=etsize+n;
         FormPea.ProgressBarPea.Position:=(100*etsize) div (tsize+paramcount);
         tsout:=datetimetotimestamp(now);
         time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
         FormPea.LabelToolsElapsed.Caption:=nicetime(inttostr(time))+' elapsed';
   ...
   if n<>0 then
      ...
      FormReport.StringGridInput.Cells[26,t]:=utf8encode(dummystr);
      end;
   ...
   if n<>0 then
      ...
      dummystr:='';
      SetString(dummystr, PChar(@sbuf[1]), n);
      FormReport.StringGridInput.Cells[27,t]:=utf8encode(dummystr);
      end;
   close(f);
   ...
   if ((mode='HEX') or (mode='LSBHEX')) then
      ...
      if mode ='HEX' then
         ...
         if CRC24_on then FormReport.StringGridInput.Cells[10,t]:=hexstr(@pgpsig,sizeof(pgpsig));
         ...
         begin
         if CRC24_on then FormReport.StringGridInput.Cells[10,t]:=hexstr(@CRC24,sizeof(CRC24));
         end;
      if Adler32_on then FormReport.StringGridInput.Cells[8,t]:=lowercase(hexstr(@adler,sizeof(Adler)));
      if CRC16_on then FormReport.StringGridInput.Cells[9,t]:=lowercase(hexstr(@CRC16,sizeof(CRC16)));
      if CRC32_on then FormReport.StringGridInput.Cells[11,t]:=lowercase(hexstr(@CRC32,sizeof(CRC32)));
      if CRC64_on then FormReport.StringGridInput.Cells[12,t]:=lowercase(hexstr(@CRC64,sizeof(CRC64)));
      if ED2K_on then
         begin
         FormReport.StringGridInput.Cells[13,t]:=lowercase(hexstr(@ED2KRes.eDonkey, sizeof(ED2KRes.eDonkey)));
         if ED2KRes.differ then FormReport.StringGridInput.Cells[13,t]:=FormReport.StringGridInput.Cells[13,t]+' / eMule: '+lowercase(hexstr(@ED2KRes.eMule, sizeof(ED2KRes.eMule)));
         end;
      if MD4_on then FormReport.StringGridInput.Cells[14,t]:=lowercase(hexstr(@MD4Digest,sizeof(MD4Digest)));
      if MD5_on then FormReport.StringGridInput.Cells[15,t]:=lowercase(hexstr(@MD5Digest,sizeof(MD5Digest)));
      if RIPEMD160_on then FormReport.StringGridInput.Cells[16,t]:=lowercase(hexstr(@RMD160Digest,sizeof(RMD160Digest)));
      if SHA1_on then FormReport.StringGridInput.Cells[17,t]:=lowercase(hexstr(@SHA1Digest,sizeof(SHA1Digest)));
      if Blake2s_on then FormReport.StringGridInput.Cells[18,t]:=lowercase(hexstr(@Blake2sDigest,sizeof(Blake2sDigest)));//if SHA224_on then FormReport.StringGridInput.Cells[18,t]:=lowercase(hexstr(@SHA224Digest,sizeof(SHA224Digest)));
      if SHA256_on then FormReport.StringGridInput.Cells[19,t]:=lowercase(hexstr(@SHA256Digest,sizeof(SHA256Digest)));
      if SHA3_256_on then FormReport.StringGridInput.Cells[20,t]:=lowercase(hexstr(@SHA3_256Digest,sizeof(SHA3_256Digest)));
      if Blake2b_on then FormReport.StringGridInput.Cells[21,t]:=lowercase(hexstr(@Blake2bDigest,sizeof(Blake2bDigest)));//if SHA384_on then FormReport.StringGridInput.Cells[21,t]:=lowercase(hexstr(@SHA384Digest,sizeof(SHA384Digest)));
      if SHA512_on then FormReport.StringGridInput.Cells[22,t]:=lowercase(hexstr(@SHA512Digest,sizeof(SHA512Digest)));
      if SHA3_512_on then FormReport.StringGridInput.Cells[23,t]:=lowercase(hexstr(@SHA3_512Digest,sizeof(SHA3_512Digest)));
      if Whirlpool_on then FormReport.StringGridInput.Cells[24,t]:=lowercase(hexstr(@WhirlDigest,sizeof(WhirlDigest)));
      end
   else
      begin
      if Adler32_on then FormReport.StringGridInput.Cells[8,t]:=base64str(@adler,sizeof(Adler));
      if CRC16_on then FormReport.StringGridInput.Cells[9,t]:=base64str(@CRC16,sizeof(CRC16));
      if CRC24_on then FormReport.StringGridInput.Cells[10,t]:=base64str(@pgpsig,sizeof(CRC24));
      if CRC32_on then FormReport.StringGridInput.Cells[11,t]:=base64str(@CRC32,sizeof(CRC32));
      if CRC64_on then FormReport.StringGridInput.Cells[12,t]:=base64str(@CRC64,sizeof(CRC64));
      if ED2K_on then
         begin
         FormReport.StringGridInput.Cells[13,t]:=base64str(@ED2KRes.eDonkey, sizeof(ED2KRes.eDonkey));
         if ED2KRes.differ then FormReport.StringGridInput.Cells[13,t]:=FormReport.StringGridInput.Cells[13,t]+' / eMule: '+base64str(@ED2KRes.eMule, sizeof(ED2KRes.eMule));
         end;
      if MD4_on then FormReport.StringGridInput.Cells[14,t]:=base64str(@MD4Digest,sizeof(MD4Digest));
      if MD5_on then FormReport.StringGridInput.Cells[15,t]:=base64str(@MD5Digest,sizeof(MD5Digest));
      if RIPEMD160_on then FormReport.StringGridInput.Cells[16,t]:=base64str(@RMD160Digest,sizeof(RMD160Digest));
      if SHA1_on then FormReport.StringGridInput.Cells[17,t]:=base64str(@SHA1Digest,sizeof(SHA1Digest));
      if Blake2s_on then FormReport.StringGridInput.Cells[18,t]:=base64str(@Blake2sDigest,sizeof(Blake2sDigest)); //if SHA224_on then FormReport.StringGridInput.Cells[18,t]:=base64str(@SHA224Digest,sizeof(SHA224Digest));
      if SHA256_on then FormReport.StringGridInput.Cells[19,t]:=base64str(@SHA256Digest,sizeof(SHA256Digest));
      if SHA3_256_on then FormReport.StringGridInput.Cells[20,t]:=base64str(@SHA3_256Digest,sizeof(SHA3_256Digest));
      if Blake2b_on then FormReport.StringGridInput.Cells[21,t]:=base64str(@Blake2bDigest,sizeof(Blake2bDigest)); //if SHA384_on then FormReport.StringGridInput.Cells[21,t]:=base64str(@SHA384Digest,sizeof(SHA384Digest));
      if SHA512_on then FormReport.StringGridInput.Cells[22,t]:=base64str(@SHA512Digest,sizeof(SHA512Digest));
      if SHA3_512_on then FormReport.StringGridInput.Cells[23,t]:=base64str(@SHA3_512Digest,sizeof(SHA3_512Digest));
      if Whirlpool_on then FormReport.StringGridInput.Cells[24,t]:=base64str(@WhirlDigest,sizeof(WhirlDigest));
      end;
   end
   ...
   if etsize>0 then FormPea.LabelToolsResult2.Caption:='Potential compression '+inttostr((etsize*100 - compsize) div etsize)+'%';
   end
   else//directory
    begin
    t:=t+1;
    ...
    FormReport.StringGridInput.Cells[0,t]:=(exp_files[k]);
    FormReport.StringGridInput.Cells[1,t]:=extractfilename(dummystr);
    FormReport.StringGridInput.Cells[2,t]:=' [folder]';
    FormReport.StringGridInput.Cells[3,t]:='';
    FormReport.StringGridInput.Cells[4,t]:='';
    FormReport.StringGridInput.Cells[5,t]:=FormatDateTime('yyyy-mm-dd hh:mm:ss', filedatetodatetime(exp_ftimes[k]));
    FormReport.StringGridInput.Cells[6,t]:=exp_fattr_dec[k];
    ctsize:=0;
    nfiles:=0;
    ...
    FormReport.StringGridInput.Cells[28,t]:='('+inttostr(ctsize)+' B) '+(inttostr(ndirs-1))+' dir(s), '+(inttostr(nfiles))+' file(s), '+nicenumber(inttostr(ctsize),0);
    if tsize>0 then FormReport.StringGridInput.Cells[29,t]:=inttostr((100*ctsize) div tsize)+'%';
    if tsize>0 then FormReport.StringGridInput.Cells[30,t]:=inttostr(length(FormReport.StringGridInput.Cells[27,t]))+FormReport.StringGridInput.Cells[27,t];
    end;
end;
FormReport.StringGridInput.AutosizeColumns;
if Adler32_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      Adler32Init(Adler);
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[8,i];
         if sdig<>'' then
            begin
      ...
      case mode of
         ...
         begin
         Adler := (Adler shr 24) or ((Adler shr 8) and $FF00) or ((Adler shl 8) and $FF0000) or (Adler shl 24);
         FormReport.StringGridInput.Cells[8,t+1]:=lowercase(hexstr(@Adler,sizeof(Adler)));
         end;
         'LSBHEX': FormReport.StringGridInput.Cells[8,t+1]:=lowercase(hexstr(@Adler,sizeof(Adler)));
         'BASE64':FormReport.StringGridInput.Cells[8,t+1]:=base64str(@Adler,sizeof(Adler));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[8]:=0;
if CRC16_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[9,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         ...
         begin
         CRC16 := swap(CRC16);
         FormReport.StringGridInput.Cells[9,t+1]:=lowercase(hexstr(@CRC16,sizeof(CRC16)));
         end;
         'LSBHEX': FormReport.StringGridInput.Cells[9,t+1]:=lowercase(hexstr(@CRC16,sizeof(CRC16)));
         'BASE64':FormReport.StringGridInput.Cells[9,t+1]:=base64str(@CRC16,sizeof(CRC16));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[9]:=0;
if CRC24_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[10,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX':
         begin
         FormReport.StringGridInput.Cells[10,t+1]:=hexstr(@pgpsig,sizeof(pgpsig));
         FormReport.StringGridInput.Cells[10,t+1]:=lowercase(hexstr(@CRC24,sizeof(CRC24)));
         end;
         'LSBHEX': FormReport.StringGridInput.Cells[10,t+1]:=hexstr(@CRC24,sizeof(CRC24));
         'BASE64': FormReport.StringGridInput.Cells[10,t+1]:=base64str(@pgpsig,sizeof(CRC24))
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[10]:=0;
if CRC32_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[11,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         ...
         begin
         CRC32 := (CRC32 shr 24) or ((CRC32 shr 8) and $FF00) or ((CRC32 shl 8) and $FF0000) or (CRC32 shl 24);
         FormReport.StringGridInput.Cells[11,t+1]:=lowercase(hexstr(@CRC32,sizeof(CRC32)));
         end;
         'LSBHEX': FormReport.StringGridInput.Cells[11,t+1]:=lowercase(hexstr(@CRC32,sizeof(CRC32)));
         'BASE64':FormReport.StringGridInput.Cells[11,t+1]:=base64str(@CRC32,sizeof(CRC32));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[11]:=0;
if CRC64_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[12,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      CRC64Final(CRC64);
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[12,t+1]:=lowercase(hexstr(@CRC64,sizeof(CRC64)));
         'BASE64':FormReport.StringGridInput.Cells[12,t+1]:=base64str(@CRC64,sizeof(CRC64));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[12]:=0;
if ED2K_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[13,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX':
         begin
         FormReport.StringGridInput.Cells[13,t+1]:=lowercase(hexstr(@ED2KRes.eDonkey, sizeof(ED2KRes.eDonkey)));
         if ED2KRes.differ then FormReport.StringGridInput.Cells[13,t+1]:=FormReport.StringGridInput.Cells[13,t+1]+' / eMule: '+lowercase(hexstr(@ED2KRes.eMule, sizeof(ED2KRes.eMule)));
         end;
         'BASE64':
         begin
         FormReport.StringGridInput.Cells[13,t+1]:=base64str(@ED2KRes.eDonkey, sizeof(ED2KRes.eDonkey));
         if ED2KRes.differ then FormReport.StringGridInput.Cells[13,t+1]:=FormReport.StringGridInput.Cells[13,t+1]+' / eMule: '+base64str(@ED2KRes.eMule, sizeof(ED2KRes.eMule));
         end;
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[13]:=0;
if MD4_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[14,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[14,t+1]:=lowercase(hexstr(@MD4Digest,sizeof(MD4Digest)));
         'BASE64':FormReport.StringGridInput.Cells[14,t+1]:=base64str(@MD4Digest,sizeof(MD4Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[14]:=0;
if MD5_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[15,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[15,t+1]:=lowercase(hexstr(@MD5Digest,sizeof(MD5Digest)));
         'BASE64':FormReport.StringGridInput.Cells[15,t+1]:=base64str(@MD5Digest,sizeof(MD5Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[15]:=0;
if RIPEMD160_on then
   if (t-td-te)>1 then
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[16,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[16,t+1]:=lowercase(hexstr(@RMD160Digest,sizeof(RMD160Digest)));
         'BASE64':FormReport.StringGridInput.Cells[16,t+1]:=base64str(@RMD160Digest,sizeof(RMD160Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[16]:=0;
if SHA1_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[17,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[17,t+1]:=lowercase(hexstr(@SHA1Digest,sizeof(SHA1Digest)));
         'BASE64':FormReport.StringGridInput.Cells[17,t+1]:=base64str(@SHA1Digest,sizeof(SHA1Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[17]:=0;
if Blake2s_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[18,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[18,t+1]:=lowercase(hexstr(@Blake2sDigest,sizeof(Blake2sDigest)));
         'BASE64':FormReport.StringGridInput.Cells[18,t+1]:=base64str(@Blake2sDigest,sizeof(Blake2sDigest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[18]:=0;
{if SHA224_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[18,t+1]:=lowercase(hexstr(@SHA224Digest,sizeof(SHA224Digest)));
         'BASE64':FormReport.StringGridInput.Cells[18,t+1]:=base64str(@SHA224Digest,sizeof(SHA224Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[18]:=0;}
if SHA256_on then
   if (t-td-te)>1 then
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[19,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[19,t+1]:=lowercase(hexstr(@SHA256Digest,sizeof(SHA256Digest)));
         'BASE64':FormReport.StringGridInput.Cells[19,t+1]:=base64str(@SHA256Digest,sizeof(SHA256Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[19]:=0;
if SHA3_256_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[20,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[20,t+1]:=lowercase(hexstr(@SHA3_256Digest,sizeof(SHA3_256Digest)));
         'BASE64':FormReport.StringGridInput.Cells[20,t+1]:=base64str(@SHA3_256Digest,sizeof(SHA3_256Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[20]:=0;
if Blake2b_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[21,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[21,t+1]:=lowercase(hexstr(@Blake2bDigest,sizeof(Blake2bDigest)));
         'BASE64':FormReport.StringGridInput.Cells[21,t+1]:=base64str(@Blake2bDigest,sizeof(Blake2bDigest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[21]:=0;
{if SHA384_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[21,t+1]:=lowercase(hexstr(@SHA384Digest,sizeof(SHA384Digest)));
         'BASE64':FormReport.StringGridInput.Cells[21,t+1]:=base64str(@SHA384Digest,sizeof(SHA384Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[21]:=0;}
if SHA512_on then
   if (t-td-te)>1 then
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[22,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[22,t+1]:=lowercase(hexstr(@SHA512Digest,sizeof(SHA512Digest)));
         'BASE64':FormReport.StringGridInput.Cells[22,t+1]:=base64str(@SHA512Digest,sizeof(SHA512Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[22]:=0;
if SHA3_512_on then
   if (t-td-te)>1 then
      begin
      FormReport.StringGridInput.Cells[0,t+1]:='* Digest *';
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[23,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[23,t+1]:=lowercase(hexstr(@SHA3_512Digest,sizeof(SHA3_512Digest)));
         'BASE64':FormReport.StringGridInput.Cells[23,t+1]:=base64str(@SHA3_512Digest,sizeof(SHA3_512Digest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[23]:=0;
if Whirlpool_on then
   if (t-td-te)>1 then
      ...
      for i:=1 to t do
         begin
         sdig:=FormReport.StringGridInput.Cells[24,i];
         if sdig<>'' then
            begin
            for j:=1 to sizeof(sdig) do sbuf[j]:=byte(sdig[j]);
      ...
      case mode of
         'HEX','LSBHEX': FormReport.StringGridInput.Cells[24,t+1]:=lowercase(hexstr(@WhirlDigest,sizeof(WhirlDigest)));
         'BASE64':FormReport.StringGridInput.Cells[24,t+1]:=base64str(@WhirlDigest,sizeof(WhirlDigest));
         end;
      end
   else
else FormReport.StringGridInput.ColWidths[24]:=0;
FormReport.StringGridInput.ColWidths[25]:=0;
if oper='LIST' then
   begin
   FormReport.StringGridInput.ColWidths[26]:=0;
   FormReport.StringGridInput.ColWidths[27]:=0;
   end;
FormReport.StringGridInput.ColWidths[30]:=0;
if FormReport.StringGridInput.ColWidths[0]>320 then FormReport.StringGridInput.ColWidths[0]:=320;
if FormReport.StringGridInput.ColWidths[1]>200 then FormReport.StringGridInput.ColWidths[1]:=200;
///check duplicates
icol:=0;
for icount:=8 to 23 do
   if FormReport.StringGridInput.ColWidths[icount]<>0 then icol:=icount;
if icol<>0 then
begin
FormPea.ProgressBarPea.Position:=95;
rc:=FormReport.StringGridInput.RowCount-1;
if rc>1 then rc:=rc-1;//digest
FormPea.LabelToolsInput.Caption:='Checking for duplicates in '+inttostr(rc)+' items';
FormPea.LabelToolsResult1.Caption:='This passage may take some time';
Application.ProcessMessages;
for icount:=1 to rc do
   begin
   dup:=0;
   if (FormReport.StringGridInput.Cells[2,icount]<>' [folder]') and (FormReport.StringGridInput.Cells[4,icount]<>'0') then
      begin
      for iver:=1 to rc do
         if FormReport.StringGridInput.Cells[icol,iver]<>FormReport.StringGridInput.Cells[icol,icount] then else dup:=dup+1;
      FormReport.StringGridInput.Cells[7,icount]:=inttostr(dup);
      end
   else FormReport.StringGridInput.Cells[7,icount]:='-';
   if icount>1024 then
      if (icount and 127) = 0 then
         begin
         FormPea.LabelToolsInput.Caption:='Checking for duplicates of '+FormReport.StringGridInput.Cells[1,icount];
         FormPea.LabelToolsResult1.Caption:='Item '+inttostr(icount)+' of '+inttostr(rc);
         Application.ProcessMessages;
         end;
   end;
end
else FormReport.StringGridInput.ColWidths[7]:=0;
///
FormReport.StringGridInput.PopupMenu:=FormReport.PopupMenu1;
FormPea.ButtonToolsCancel.visible:=false;
FormPea.ProgressBarPea.Position:=100;
FormPea.LabelToolsInput.Caption:='Checked ('+moded+') '+nicenumber(inttostr(tsize),0)+' ['+inttostr(tsize)+' B]';
FormPea.LabelToolsResult1.Caption:='Processed '+inttostr(t)+' of '+inttostr(ntotalexp)+' items: '+inttostr(t-td-te)+' files, '+inttostr(td)+' directories, '+inttostr(te)+' errors';
try
if t>1 then FormPea.LabelToolsResult2.Caption:=
   ...
   FormPea.LabelToolsResult2.Caption:='Size '+nicenumber(inttostr(smax),0)+', date '+FormatDateTime('yyyy-mm-dd hh:mm:ss', filedatetodatetime(dmax));
except end;
if etsize>0 then FormPea.LabelToolsResult2.Caption:=FormPea.LabelToolsResult2.Caption+', potential compression '+inttostr((etsize*100 - compsize) div etsize)+'%';
tsout:=datetimetotimestamp(now);
time:=((tsout.date-tsin.date)*24*60*60*1000)+tsout.time-tsin.time;
if time>0 then
   begin
   speed:=(tsize * 1000) div time;
   FormPea.LabelToolsElapsed.Caption:=nicetime(inttostr(time))+' total time @ '+nicenumber(inttostr(speed),0)+'/s';
   end
else FormPea.LabelToolsElapsed.Caption:='';
FormReport.LabelReport1.Caption:=FormPea.LabelToolsInput.Caption;
FormReport.LabelReport2.Caption:=FormPea.LabelToolsResult1.Caption;
FormReport.LabelReport3.Caption:=FormPea.LabelToolsResult2.Caption;
FormReport.LabelReport4.Caption:=FormPea.LabelToolsElapsed.Caption;
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormPea.ButtonClosePea.Visible:=true;
FormPea.LabelOpen.Visible:=true;
FormPea.LabelOpen.Enabled:=false;
...
exitcode:=0;
case specop of
   'CRC32SAVE': begin FormReport.StringGridInput.Col:=11; save_hashfn; halt; end;
   'MD5SAVE': begin FormReport.StringGridInput.Col:=15; save_hashfn; halt; end;
   'SHA1SAVE': begin FormReport.StringGridInput.Col:=17; save_hashfn; halt; end;
   'SHA256SAVE': begin FormReport.StringGridInput.Col:=19; save_hashfn; halt; end;
   'BLAKE2BSAVE': begin FormReport.StringGridInput.Col:=21; save_hashfn; halt; end;
   'SHA256G': begin FormReport.StringGridInput.Col:=19; FormReport.MenuItem5Click(nil); halt; end;
   'SHA256J': begin FormReport.StringGridInput.Col:=19; FormReport.MenuItem6Click(nil); halt; end;
   'SHA256V': begin FormReport.StringGridInput.Col:=19; FormReport.MenuItem4Click(nil); halt; end;
end;
end;
...
FormPea.PanelPW.height:=2;
FormReport.NotebookReport.PageIndex:=0;
FormReport.Caption:='Environment variables';
FormPea.Caption:='Environment variables';
FormPea.LabelToolsInput.Caption:='';
FormPea.ProgressBarPea.Position:=0;
FormReport.InputT.Caption:='Environment variables';
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormReport.StringGridInput.ColCount:=2;
FormReport.StringGridInput.Cells[0,0]:='N';
FormReport.StringGridInput.Cells[1,0]:='Variable';
FormReport.StringGridInput.Rowcount:=GetEnvironmentVariableCount+2;
for i:=0 to GetEnvironmentVariableCount do
   begin
   FormReport.StringGridInput.Cells[0,i+1]:=inttostr(i);
   FormReport.StringGridInput.Cells[1,i+1]:=GetEnvironmentString(i);
   end;
FormReport.StringGridInput.AutosizeColumns;
FormPea.ProgressBarPea.Position:=100;
FormReport.LabelReport1.Caption:=FormPea.Caption;
...
{$IFDEF MSWINDOWS}
s:=(paramstr(3));
FormPea.Caption:='MoTW';
FormPea.ImageInfoTools.Picture.Bitmap:=Binfo;
FormPea.LabelToolsInput.Caption:='Checking MoTW. May take some time, please wait...';
FormPea.ProgressBarPea.Position:=1;
FormPea.ButtonToolsCancel.visible:=true;
Application.ProcessMessages;
for i:=2 to paramcount do
   begin
   FormPea.ProgressBarPea.Position:=((i-1)*100) div paramcount;
   expand(paramstr(i),exp_files,exp_fsizes,exp_ftimes,exp_fattr,exp_fattr_dec,nfound);
   if (nfound=0) and (pos('D',exp_fattr_dec[0])=0) then //file
      begin
      if toolactioncancelled=true then
         begin
         FormPea.LabelToolsInput.Caption:='Zone.Identifier (Mark of The Web) test cancelled';
         FormPea.ProgressBarPea.Position:=100;
         FormPea.ButtonToolsCancel.visible:=false;
         cancelcheck;
         exit;
         end;
      FormPea.LabelToolsResult1.Caption:=inttostr(i-1)+'/'+inttostr(paramcount-1)+' '+exp_files[0];
      Application.ProcessMessages;
      if checkmotwf(exp_files[0])=true then
         begin
         FormPea.ImageInfoTools.Picture.Bitmap:=Bok;
         FormPea.LabelToolsInput.Caption:='Zone.Identifier (Mark of The Web) found in input file(s)';
         FormPea.ProgressBarPea.Position:=100;
         FormPea.ButtonToolsCancel.visible:=false;
         FormPea.ButtonToolsOK.visible:=true;
         Application.ProcessMessages;
         exitcode:=1;
         exit;
         end;
      ...
      for j:=0 to nfound-1 do
         if pos('D',exp_fattr_dec[j])<>0 then
            ...
            FormPea.LabelToolsResult1.Caption:=inttostr(i-1)+'/'+inttostr(paramcount-1)+' '+exp_files[j];
            Application.ProcessMessages;
...
{$IFDEF MSWINDOWS}
s:=(paramstr(3));
FormPea.Caption:='MoTW (full report)';
FormPea.ImageInfoTools.Picture.Bitmap:=Binfo;
FormPea.LabelToolsInput.Caption:='Checking MoTW (full report). May take some time, please wait...';
FormPea.ProgressBarPea.Position:=1;
FormPea.ButtonToolsCancel.visible:=true;
Application.ProcessMessages;
FormReport.InputT.Caption:='Input';
FormReport.Caption:='MoTW';
FormReport.StringGridInput.ColCount:=2;
FormReport.StringGridInput.Cells[0,0]:='Name';
FormReport.StringGridInput.Cells[1,0]:='MoTW';
FormReport.NotebookReport.PageIndex:=0;
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormReport.StringGridInput.RowCount:=1;
ntotal:=0;
...
for i:=2 to paramcount do
   ...
   if (nfound=0) and (pos('D',exp_fattr_dec[0])=0) then //file
      ...
      Application.ProcessMessages;
      ntotal:=ntotal+1;
      FormReport.StringGridInput.RowCount:=ntotal+1;
      FormReport.StringGridInput.Cells[0,ntotal]:=exp_files[0];
      if checkmotwf(exp_files[0])=true then
         begin
         FormPea.ImageInfoTools.Picture.Bitmap:=Bok;
         FormPea.LabelToolsInput.Caption:='Zone.Identifier (Mark of The Web) found in input file(s)';
         FormReport.StringGridInput.Cells[1,ntotal]:='Yes';
         nmotw:=nmotw+1;
         end
      else
         begin
         FormReport.StringGridInput.Cells[1,ntotal]:='No';
         end;
      end
      ...
      for j:=0 to nfound-1 do
         if pos('D',exp_fattr_dec[j])=0 then //file
            ...
            ntotal:=ntotal+1;
            FormReport.StringGridInput.RowCount:=ntotal+1;
            FormReport.StringGridInput.Cells[0,ntotal]:=exp_files[j];
            if checkmotwd(exp_files[j])=true then
               begin
               FormPea.ImageInfoTools.Picture.Bitmap:=Bok;
               FormPea.LabelToolsInput.Caption:='Zone.Identifier (Mark of The Web) found in input file(s)';
...
FormPea.Visible:=false;
FormReport.StringGridInput.AutosizeColumns;
FormReport.LabelReport1.Caption:='';
FormReport.LabelReport2.Caption:='';
FormReport.LabelReport3.Caption:='';
FormReport.LabelReport4.Caption:='';
try FormReport.LabelReport2.Caption:='Zone.Identifier (Mark of The Web) found in '+inttostr(nmotw)+' of '+inttostr(ntotal)+' files'; except end;
exitcode:=nmotw;
{$ENDIF}
end;
...
mode:=(paramstr(2));
s:=(paramstr(3));
FormPea.Caption:=mode;
FormPea.LabelToolsInput.Caption:='Listing '+s+' may take some time, please wait...';
FormPea.ProgressBarPea.Position:=5;
Application.ProcessMessages;
expand(s,exp_files,exp_fsizes,exp_ftimes,exp_fattr,exp_fattr_dec,nfound);
FormPea.ProgressBarPea.Position:=50;
Application.ProcessMessages;
nsize:=0;
compsize:=0;
compest:=0;
...
smin:=exp_fsizes[0];
if nfound=0 then nfound:=1;
FormPea.LabelToolsResult1.Caption:='Found: '+inttostr(nfound);
FormPea.ProgressBarPea.Position:=60;
Application.ProcessMessages;
...
FormPea.LabelToolsResult2.Caption:='Total size: '+inttostr(nsize)+' B';
FormPea.ProgressBarPea.Position:=70;
Application.ProcessMessages;
FormReport.InputT.Caption:='Input';
if upcase(mode)='INFO' then FormReport.Caption:='Info'
else FormReport.Caption:='List';
FormReport.StringGridInput.ColCount:=4;
FormReport.StringGridInput.Cells[0,0]:='Name';
FormReport.StringGridInput.Cells[1,0]:='Size (B)';
FormReport.StringGridInput.Cells[2,0]:='Date/time';
FormReport.StringGridInput.Cells[3,0]:='Attributes';
{$IFDEF MSWINDOWS}FormReport.OutputT.TabVisible:=false;{$ENDIF}FormReport.NotebookReport.ShowTabs:=false;
FormReport.StringGridInput.RowCount:=nfound+1;
for k:=0 to nfound-1 do
    begin
    FormReport.StringGridInput.Cells[0,k+1]:=exp_files[k];
    FormReport.StringGridInput.Cells[1,k+1]:=inttostr(exp_fsizes[k]);
    try FormReport.StringGridInput.Cells[2,k+1]:=datetimetostr(filedatetodatetime(exp_ftimes[k])); except FormReport.StringGridInput.Cells[2,k+1]:='unknown'; end;
    FormReport.StringGridInput.Cells[3,k+1]:=exp_fattr_dec[k];
    end;
FormPea.ProgressBarPea.Position:=100;
Application.ProcessMessages;
FormReport.StringGridInput.AutosizeColumns;
FormReport.LabelReport1.Caption:=s;
FormReport.LabelReport2.Caption:='';
FormReport.LabelReport3.Caption:='';
FormReport.LabelReport4.Caption:='';
if upcase(mode)='INFO' then
   begin
   try FormReport.LabelReport2.Caption:='Found: '+inttostr(nfound)+' objects (newer: '+datetimetostr(filedatetodatetime(dmax))+', older: '+datetimetostr(filedatetodatetime(dmin))+')'; except FormReport.LabelReport2.Caption:='Found: '+inttostr(nfound)+' objects'; end;
   FormReport.LabelReport3.Caption:='Total size: '+nicenumber(inttostr(nsize),0)+' (larger: '+nicenumber(inttostr(smax),0)+', smaller: '+nicenumber(inttostr(smin),0)+');';
   if nsize<>0 then FormReport.LabelReport3.Caption:=FormReport.LabelReport3.Caption+' potential compression: '+inttostr((nsize*100 - compsize) div nsize)+'%';//+nicenumber(inttostr(compsize div 100))+' ('+inttostr(compsize div nsize)+'%)';
   end;
FormReport.Visible:=true;
FormPea.Visible:=false;
exitcode:=0;
...
if directoryexists(paramstr(2)) then
   begin
   FormPea.LabelToolsInput.Caption:=paramstr(2)+' is a directory, cannot be previewed';
   exit;
   end;
FormPea.PanelPW.height:=2;
FormReport.NotebookReport.PageIndex:=0;
FormPea.Caption:='Text preview';
FormReport.Caption:=extractfilename(paramstr(2));
needclose:=true;
noreportdetails:=true;
FormPea.LabelToolsInput.Caption:=(paramstr(2));
FormPea.LabelToolsResult1.Caption:='';
FormPea.LabelToolsResult2.Caption:='';
FormPea.ProgressBarPea.Position:=0;
FormReport.StringGridInput.Clear;
FormReport.StringGridInput.RowCount:=1;
sizea:=0;
if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_rep(FormReport.LabelTitleREP2,FormReport.ShapeTitleREPb2) else clicklabel_rep(FormReport.LabelTitleREP2,FormReport.ShapeLinkREP2);
FormReport.LabelTitleREP1.Enabled:=False;
FormReport.LabelTitleREP2.Enabled:=False;
FormReport.ShapeTitleREPb1.Visible:=False;
FormReport.ShapeTitleREPb2.Visible:=False;
FormReport.LabelTitleREP1.Visible:=False;
FormReport.LabelTitleREP2.Visible:=False;
FormReport.Shapelinkrep1.Visible:=False;
FormReport.Shapelinkrep2.Visible:=False;
FormReport.PanelTitleREPTabAlign.Visible:=False;
FormReport.LabelSaveCsv.Visible:=False;
FormReport.LabelSaveCsv1.Visible:=False;
FormReport.LabelSaveTsv.Visible:=False;
FormReport.LabelSave1.Visible:=False;
FormReport.LabelSave2.Visible:=False;
FormReport.LabelSave3.Visible:=False;
FormReport.LabelEncoding.Visible:=True;
FormReport.EditFindText.Visible:=True;
FormReport.ButtonFindText.Visible:=True;
FormReport.MemoReport.ReadOnly:=True;
FormReport.MemoReport.Hint:='';
try
srcfilesize((paramstr(2)),sizea);
if sizea=0 then
   begin
...
setcurrentdir(extractfilepath((paramstr(2))));
FormPea.ProgressBarPea.Position:=5;
UnitReport.intextfile:=paramstr(2);
UnitReport.intextfileenc:=TEncoding.Default;
...
FormPea.LabelToolsResult1.Caption:='Size '+nicenumber(inttostr(sizea),0)+' ('+inttostr(sizea)+' B)';
UnitReport.sizeastr:=FormPea.LabelToolsResult1.Caption;
FormReport.StringGridInput.RowCount:=1024;
application.ProcessMessages;
FormPea.ProgressBarPea.Position:=95;
application.ProcessMessages;
FormPea.ProgressBarPea.Position:=100;
...
FormPea.LabelToolsResult1.Caption:='Size '+nicenumber(inttostr(sizea),0)+' ('+inttostr(sizea)+' B)';
FormReport.StringGridInput.RowCount:=(sizea div 16) +2;
total:=0;
prows:=1;
wrbytes:=0;
application.ProcessMessages;
repeat
   ...
   for y:=0 to nrows-1 do
      ...
      FormReport.StringGridInput.Cells[0,y+prows]:=offs;
      astr:='';
      for x:=0 to 15 do
         begin
         i:=i+1;
      ...
      wrbytes:=wrbytes+16;
      FormReport.StringGridInput.Cells[1,y+prows]:=hexs1;
      FormReport.StringGridInput.Cells[2,y+prows]:=astr;
      end;
   inc(total,numreada);
   prows:=prows+nrows;
   FormPea.ProgressBarPea.Position:=(total*100) div sizea;
   Application.ProcessMessages;
...
if (upcase(paramstr(7))='TRIATS') or (upcase(paramstr(7))='TRITSA') or (upcase(paramstr(7))='TRISAT') or
   ...
   if (upcase(paramstr(8))='INTERACTIVE') or (upcase(paramstr(8))='INTERACTIVE_REPORT') then
      ...
      FormPea.LabelConfirm.Visible:=true;
      FormPea.EditConfirm.Visible:=true;
      FormPea.LabelHintPW.Visible:=true;
      FormPea.LabelEnt.Visible:=true;
      FormPea.LabelSampleEnt.Visible:=true;
      FormPea.ImageInfoPW.Visible:=true;
      exit;
      end;
interacting:=false;
end;
...
if upcase(paramstr(3))='ASK' then
   begin
   FormPea.PanelRFSinteractive.visible:=true;
   FormPea.Caption:='Split file';
   exit;
   end;
interacting:=false;
...
FormPea.Visible:=true;
FormPea.PanelRFSinteractive.visible:=false;
FormPea.PanelTools.visible:=true;
interacting:=false;
end;
...
procedure TFormPea.ButtonClosePeaClick(Sender: TObject);
begin
Close;
end;

procedure TFormPea.ButtonEncryptCancelClick(Sender: TObject);
begin
halt(-4);
end;

procedure TFormPea.ButtonPWOKClick(Sender: TObject);
begin
if (EditPW.Text='') and (EditConfirm.Text='') then pw:='default'
else
   if (upcase(paramstr(1))='PEA') and (EditPW.Text<>EditConfirm.Text) then
      begin
      MessageDlg('Passwords doesn''t match, please retype "Passphrase" and "Confirm" fields', mtWarning, [mbOK], 0);
      exit;
      end
   else pw:=EditPW.Text;
if LabelKeyFileName.Caption='<none>' then keyfile_name:='NOKEYFILE'
else keyfile_name:=LabelKeyFileName.Caption;
PanelPW.Visible:=false;
interacting:=false;
end;

procedure TFormPea.ButtonPWCancelClick(Sender: TObject);
begin
FormPea.Close;
end;

procedure TFormPea.ButtonRFSinteractive1Click(Sender: TObject);
begin
FormPea.Close;
end;

procedure TFormPea.ButtonRFSinteractiveClick(Sender: TObject);
begin
   case ComboBoxSplitPresets.ItemIndex of
      0: begin
         try
            vol_size:=SpinEditSplitSize.Value;
            case ComboBoxSplitSizeunit.ItemIndex of
   ...
   case ComboSplitControlalgo.ItemIndex of //volume checks
...
procedure TFormPea.ButtonToolsOKClick(Sender: TObject);
begin
FormPea.Close;
end;

procedure TFormPea.ButtonToolsCancelClick(Sender: TObject);
begin
toolactioncancelled:=true;
end;

procedure TFormPea.ButtonUtilsCancelClick(Sender: TObject);
begin
FormPea.Close;
end;

procedure TFormPea.ButtonUtilsResetClick(Sender: TObject);
begin
ListMemoUtilsInput.Clear;
end;
...
repeat
   ...
   inc(i);
   if s<>'' then FormPea.ComboBoxUnits.Items.Add(s);
until length(s)=0;
nunits:=nunits-1;
{$ENDIF}
end;

procedure TFormPea.ButtonUtilsOKClick(Sender: TObject);
var
  cl,bin_name,in_param:ansistring;
  i:integer;
  P:TProcessUTF8;
begin
FormReport.StringGridInput.PopupMenu:=nil;
...
case ComboBoxUtils.ItemIndex of
   ...
   15: cl:=bin_name+' RFS AUTONAME ASK NONE BATCH '+stringdelim(ListMemoUtilsInput.Lines[0]); //one file
   16: cl:=bin_name+' RFJ '+stringdelim(ListMemoUtilsInput.Lines[0])+' BATCH AUTONAME'; //one file (strictly)
   17: cl:=bin_name+' COMPARE '+in_param; //two files or one file (ask for second file, ignores more files)
   18: cl:=bin_name+' HEXPREVIEW '+stringdelim(ListMemoUtilsInput.Lines[0]);  //one file
   19: cl:=bin_name+' TEXTPREVIEW '+stringdelim(ListMemoUtilsInput.Lines[0]);  //one file
   20: begin
      if MessageDlg('Do you want to securely delete selected file(s)? The operation can''t be undone and files will be not recoverable', mtWarning, [mbYes,mbNo], 0)=6 then
         ...
         repeat
         if length(ListMemoUtilsInput.Lines[i])>1 then
            if not fileexists(ListMemoUtilsInput.Lines[i]) then ListMemoUtilsInput.Lines.Delete(i)
            else i:=i+1;
         until i>=ListMemoUtilsInput.Lines.Count;
         exit;
         end
      else exit;
      end;
...
begin
change_imagesplit;
if ComboBoxSplitPresets.ItemIndex = 0 then
   begin
   SpinEditSplitSize.Visible:=true;
   ComboBoxSplitSizeunit.Visible:=true;
   end
else
   begin
   SpinEditSplitSize.Visible:=false;
   ComboBoxSplitSizeunit.Visible:=false;
   end;
end;
end;

procedure TFormPea.ComboBoxSplitPresetsChange(Sender: TObject);
begin
ComboBox1_onchange;
end;
...
with FormPea do
begin
ListMemoUtilsInput.Enabled:=True;
ListMemoUtilsInput.Visible:=True;
LabelUtilsInput.Visible:=True;
LabelOpenfileSearch.Visible:=True;
LabelOpenfileSelect.Visible:=True;
LabelOpenfileSep.Visible:=True;
...
with FormPea do
begin
ListMemoUtilsInput.Enabled:=False;
ListMemoUtilsInput.Clear;
ListMemoUtilsInput.Visible:=False;
LabelUtilsInput.Visible:=False;
LabelOpenfileSearch.Visible:=False;
LabelOpenfileSelect.Visible:=False;
LabelOpenfileSep.Visible:=False;
...
procedure TFormPea.ComboBoxUtilsChange(Sender: TObject);
begin
case ComboBoxUtils.ItemIndex of
   0: enabledropmenu;
   1: enabledropmenu;
...
procedure TFormPea.EditPWKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState
  );
var
   pw,pr:ansistring;
   pw_strength:dword;
   pw_rating:byte;
begin
if LabelKeyFileName.Caption[1]<>'<' then
   begin
   ShapePW.Width:=240;
   ShapePW.Brush.Color:=PBLUE;
   LabelPS.Caption:='Using KeyFile';
   end
else
   begin
   pw:=EditPW.Text;
   ...
   if (pw_strength>8) then
      if (pw_strength<240) then ShapePW.Width:=pw_strength
      else ShapePW.Width:=240
   else ShapePW.Width:=8;
   if pw_strength<24 then ShapePW.Brush.Color:=PRED
   else
      if pw_strength<48 then ShapePW.Brush.Color:=PYELLOW
      else
         if pw_strength<72 then ShapePW.Brush.Color:=PLGREEN
         else ShapePW.Brush.Color:=PGREEN;
   case ShapePW.Brush.Color of
      PRED: pr:='Weak';
      PYELLOW: pr:='Poor';
      PLGREEN: pr:='Adequate';
      PGREEN: pr:='Good';
      end;
   LabelPS.Caption:=pr+' ('+inttostr(pw_strength)+')';
   end;
end;

procedure TFormPea.FormClose(Sender: TObject; var CloseAction: TCloseAction);
var
   randf: file of byte;
   randarr: TKey2048;
begin
...
if OpenDialogKeyFile.Execute then
   if OpenDialogKeyFile.FileName<>'' then
      begin
      LabelKeyFileName.Caption:=OpenDialogKeyFile.FileName;
      ShapePW.Width:=240;
      ShapePW.Brush.Color:=PBLUE;
      LabelPS.Caption:='Using KeyFile';
      end;
...
//valorize all captions, hints, TStrings Items
   ...
   if evalcolor(clWindow)>128 then
      begin
      FormPea.ImageListPea.getbitmap(0,Bfd);
      FormPea.ImageListPea.getbitmap(1,Bmail);
      FormPea.ImageListPea.getbitmap(2,Bhd);
      FormPea.ImageListPea.getbitmap(3,Bdvd);
      FormPea.ImageListPea.getbitmap(4,Binfo);
      FormPea.ImageListPea.getbitmap(5,Blog);
      FormPea.ImageListPea.getbitmap(6,Bok);
      FormPea.ImageListPea.getbitmap(7,Bcancel);
      FormPea.ImageListPea.getbitmap(8,Butils);
      FormPea.ImageListPea.getbitmap(9,Badmin);
      end
   else
      begin
      FormPea.ImageListPea_dark.getbitmap(0,Bfd);
      FormPea.ImageListPea_dark.getbitmap(1,Bmail);
      FormPea.ImageListPea_dark.getbitmap(2,Bhd);
      FormPea.ImageListPea_dark.getbitmap(3,Bdvd);
      FormPea.ImageListPea_dark.getbitmap(4,Binfo);
      FormPea.ImageListPea_dark.getbitmap(5,Blog);
      FormPea.ImageListPea_dark.getbitmap(6,Bok);
      FormPea.ImageListPea_dark.getbitmap(7,Bcancel);
      FormPea.ImageListPea_dark.getbitmap(8,Butils);
      FormPea.ImageListPea_dark.getbitmap(9,Badmin);
      end;
   ...
   FormPea.peautilsbtn.Glyph:=Butils;
   FormPea.pmrunasadmin.Bitmap:=Badmin;
   FormPea.ImageInfoEncrypt.Picture.Bitmap:=Binfo;
   FormPea.ImageInfoDecrypt.Picture.Bitmap:=Binfo;
   FormPea.ImageInfoPW.Picture.Bitmap:=Binfo;
   FormPea.ImageInfoTools.Picture.Bitmap:=Binfo;
   FormPea.ImageUtils.Picture.Bitmap:=Binfo;
   //FormPea.ButtonPWOK.Glyph:=Bok;
   //FormPea.ButtonPWCancel.Glyph:=Bcancel;
   //FormPea.ButtonEncryptCancel.Glyph:=Bcancel;
   //FormPea.ButtonDecryptCancel.Glyph:=Bcancel;
   //FormPea.buttonrfsinteractive.Glyph:=Bok;
   //FormPea.buttonrfsinteractive1.Glyph:=Bcancel;
   //FormPea.buttonutilsok.Glyph:=Bok;
   //FormPea.buttonutilscancel.Glyph:=Bcancel;
   //FormPea.buttontoolscancel.Glyph:=Bcancel;
   except
...
procedure TFormPea.FormCreate(Sender: TObject);
...
try
   ...
   readln(conf,color4);
   readln(conf,color5);
   readln(conf,dummy); decodebintheming(dummy,gridaltcolor,highlighttabs,dummyint,dummyint,dummyint,dummyint,alttabstyle,ensmall,contrast,decostyle);
...
if (opacity<100) then
   begin
   FormPea.AlphaBlend:=true;
   FormPea.AlphaBlendValue:=255+opacity-100;
   end;
...
for i:=0 to FormPea.ListMemoUtilsInput.Lines.Count-1 do
   if FormPea.ListMemoUtilsInput.Lines[i]=name then
      begin
      testname:=1;
      exit;
      end;
...
procedure TFormPea.FormDropFiles(Sender: TObject;
...
FormReport.PanelTitleRep.Height:=tabheight;
FormReport.ShapeTitleRepb1.Height:=tablabelheight;
FormReport.ShapeTitleRepb2.Height:=tablabelheight;
FormReport.PanelSpReport.Height:=tabheightl;
PanelUtilsTitle.Height:=tabheight;
PanelSpUtils.Height:=tabheightl;
PanelSpPW.Height:=tabheightl;
PanelSpSplit.Height:=tabheightl;
pbarhsmall:=4*qscale div 100;
FormReport.Shapelinkrep1.Height:=pbarhsmall+2;
FormReport.Shapelinkrep2.Height:=pbarhsmall+2;
//grid
rowheight:=((16+2+pspacing) * qscale) div 100;
FormReport.StringGridInput.DefaultRowHeight:=rowheight;
FormReport.StringGridOutput.DefaultRowHeight:=rowheight;
end;
end;

procedure TFormPea.FormShow(Sender: TObject);
var
...
if pzooming<>100 then
   begin
   FormPea.ScaleBy(pzooming,100);
   FormReport.ScaleBy(pzooming,100);
   end;
...
FormReport.LabelSaveTxt.Font.Color:=ptextaccent;
FormReport.LabelSaveTsv.Font.Color:=ptextaccent;
FormReport.LabelSaveCsv.Font.Color:=ptextaccent;
FormReport.LabelEncoding.Font.Color:=ptextaccent;
case highlighttabs of
   0: begin
      PanelUtilsTitle.Color:=stringtocolor(color2);
      FormReport.PanelTitleREP.Color:=stringtocolor(color2);
      UnitReport.tabpencol:=StringToColor(color2);
      UnitReport.tablowcol:=psilver;
      UnitReport.tabbrushcol:=StringToColor(colmid);
      UnitReport.tabbrushhighcol:=StringToColor(colvlow);
      end;
   1: begin
      PanelUtilsTitle.Color:=stringtocolor(color2);
      FormReport.PanelTitleREP.Color:=stringtocolor(color2);
      UnitReport.tabpencol:=StringToColor(color2);
      UnitReport.tablowcol:=plblue;
      UnitReport.tabbrushcol:=pvvlblue;
      UnitReport.tabbrushhighcol:=pvvvlblue;
      end;
   2: begin
      PanelUtilsTitle.Color:=stringtocolor(collow);
      FormReport.PanelTitleREP.Color:=stringtocolor(collow);
      UnitReport.tabpencol:=StringToColor(collow);
      UnitReport.tablowcol:=psilver;
      UnitReport.tabbrushcol:=StringToColor(colmid);
      UnitReport.tabbrushhighcol:=StringToColor(colvlow);
      end;
   3: begin
      ...
      FormReport.PanelTitleREP.Color:=stringtocolor(colmid);
      UnitReport.tabpencol:=StringToColor(colmid);
      UnitReport.tablowcol:=psilver;
      UnitReport.tabbrushcol:=StringToColor(colhigh);
      UnitReport.tabbrushhighcol:=StringToColor(collow);
      end;
   4: begin
      PanelUtilsTitle.Color:=pvvvlblue;
      FormReport.PanelTitleREP.Color:=pvvvlblue;
      UnitReport.tabpencol:=pvvvlblue;
      UnitReport.tablowcol:=plblue;
      UnitReport.tabbrushcol:=pvvlblue;
      UnitReport.tabbrushhighcol:=pvvvvlblue;
      end;
   5: begin
      PanelUtilsTitle.Color:=pvvlblue;
      FormReport.PanelTitleREP.Color:=pvvlblue;
      UnitReport.tabpencol:=pvvlblue;
      UnitReport.tablowcol:=plblue;
      UnitReport.tabbrushcol:=pvlblue;
      UnitReport.tabbrushhighcol:=pvvvlblue;
      end;
   end;
if gridaltcolor=1 then
   begin
   FormReport.StringGridInput.AlternateColor:=stringtocolor(collow);
   FormReport.StringGridOutput.AlternateColor:=stringtocolor(collow);
   end
else
   begin
   FormReport.StringGridInput.AlternateColor:=stringtocolor(color2);
   FormReport.StringGridOutput.AlternateColor:=stringtocolor(color2);
   end;
if (alttabstyle<>1) and (alttabstyle<>4) then clicklabel_rep(FormReport.LabelTitleREP1,FormReport.ShapeTitleREPb1) else clicklabel_rep(FormReport.LabelTitleREP1,FormReport.ShapeLinkREP1);
if paramcount>0 then
   ...
   if upcase(paramstr(1))='PEAUTILS' then
      ...
      FormPea.Visible:=True;
      FormPea.ComboBoxUtils.ItemIndex:=kfun;
      FormPea.ComboBoxUtilsChange(nil);
      for i:=3 to paramcount do
         FormPea.ListMemoUtilsInput.Append(paramstr(i));
      except
      FormPea.ComboBoxUtilsChange(nil);
      end;
      end
      ...
      if funutil=0 then FormPea.PanelUtils.visible:=false
      else
         begin
         kfun:=12;
         FormPea.Visible:=True;
...
procedure TFormPea.ImageUtilsClick(Sender: TObject);
begin
...
procedure TFormPea.LabelLogPeaClick(Sender: TObject);
begin
FormReport.Visible:=true;
FormReport.WindowState:=wsNormal;
end;
...
if s<>'' then
   {$IFDEF MSWINDOWS}
   w:=utf8decode(s);
   cp_open:=ShellExecuteW(FormPea.Handle, PWideChar ('open'), PWideChar(w), PWideChar (''), PWideChar (''), 1);//all Windows from 95 and NT3.1
   if cp_open<33 then
      cp_open:=shellexecuteW(FormPea.handle,PWideChar('open'),PWideChar('RUNDLL32.EXE'),PWideChar('shell32.dll,OpenAs_RunDLL '+w),PWideChar (''), 1);
   {$ENDIF}
...
begin
{$IFDEF MSWINDOWS}
shellexecutew(FormPea.handle, PWideChar('find'), PWideChar(''), PWideChar(''), PWideChar (''), SW_SHOWNORMAL);
{$ENDIF}
...
procedure TFormPea.LabelOpenfileSearchClick(Sender: TObject);
begin
cp_search(desk_env);
end;

procedure TFormPea.LabelOpenfileSelectClick(Sender: TObject);
var i:integer;
begin
   if OpenDialogInput.execute then
      if OpenDialogInput.FileName<>'' then
         begin
         for i := 0 to OpenDialogInput.Files.Count-1 do
            begin
            ListMemoUtilsInput.Append(OpenDialogInput.Files[i]);
            end;
         end;
end;

procedure TFormPea.mainmenuhelpClick(Sender: TObject);
begin
cp_open(FIRSTDOM+'peazip-help.html',desk_env);
end;

procedure TFormPea.pmupdatesClick(Sender: TObject);
begin
cp_open(FIRSTDOM,desk_env);
end;

procedure TFormPea.pmdonationsClick(Sender: TObject);
begin
cp_open(FIRSTDOM+'donations.html',desk_env);
end;

procedure TFormPea.PanelPWMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var
   i:integer;
   st:string;
...
procedure TFormPea.peautilsbtnClick(Sender: TObject);
var
   pp:TPoint;
begin
...
procedure TFormPea.pmhelpClick(Sender: TObject);
begin
cp_open(FIRSTDOM+'peazip-help.html',desk_env);
end;

procedure TFormPea.pmrunasadminClick(Sender: TObject);
var
   w:widestring;
begin
{$IFDEF MSWINDOWS}
w:=utf8decode('"'+executable_path+'pea.exe"');
ShellExecuteW(FormPea.Handle, PWideChar ('runas'), PWideChar(w), PWideChar (''), PWideChar (''), SW_SHOWNORMAL);
Close;
{$ENDIF}
end;

procedure TFormPea.TimerActionTimer(Sender: TObject); //gives the time to draw the UI before the CPU intensive task begin
begin
